...
- ROY unpaid-order Stripe-expiry reconciliation is merged, deployed, and live (2026-08-10):
  ...
  - follow-up branch `codex/unpaid-deploy-exact-image` hardens this race by publishing an immutable `git-${GITHUB_SHA}` ECR tag, requiring its digest to match `latest` at build completion, and making the unpaid deploy wait up to ten minutes for that exact merge tag before any task-definition or schedule mutation
  ...
  - Next exact step: merge the exact-image follow-up after CI, then require its automatic build to emit `ECR_EXACT_IMAGE_OK` and its automatic dry-run deploy to emit `ECR_EXACT_IMAGE_RESOLVED`, the full Fargate hard-gate context, localhost marker, and `DEPLOY_UNPAID_CANCELLATION_OK`; keep `execute_now=false` because a real smoke would send cancellation emails for the audited backlog
