...
- ROY Micro SD 64GB cross-shop identity fix is merged and its ROY-only deploy workflow is being hardened before retry (2026-07-15):
  ...
  - deploy hardening branch `codex/roy-deploy-ecr-idempotency` now preserves exact manifest bytes, verifies the resulting tag digest, and treats a concurrent `ImageAlreadyExistsException` as success only after the tag resolves to the expected digest; it retains preservation tags for both scheduled images because the projects share ECR `latest`, but does not change the non-selected project's schedule, task definition, reporting data, or service
  - validation after the follow-up review: workflow YAML, extracted deploy-script `bash -n`, exact ECR manifest digest round-trip, full suite `134` tests, reporting QA smoke, and `git diff --check` pass
