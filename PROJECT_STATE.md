...
- The verifier's first Windows CI exposed a test-only path alias mismatch: its expected temporary path was not resolved, unlike the helper's canonical output path. Normalize the expected parent on both platforms; the runtime path containment check remains unchanged. Re-run both platform checks before merging the verification PR.
