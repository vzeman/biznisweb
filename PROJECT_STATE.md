...
- VEVO GrowthBook Pro rollout has a concrete, read-only preflight, dated baseline, and execution contract on branch `codex/vevo-growthbook` (`2026-08-20`):
  ...
  - Next exact step: build the exact temporary Preview tag from the authenticated GrowthBook client key and active collector endpoint, create an isolated unpublished GTM workspace, and complete the consent/control Preview checklist. Production allocation stays `0%`; Pro p75 metrics and any GTM publish remain blocked pending their separate gates
