...
- Live schema inspection and current official documentation expose supported GraphQL `preinvoiceOrder` and `finalizeInvoice` operations. The selected correction uses those operations once per phase with explicit empty notification requests, fresh associated-document readbacks and durable `preparing`/`prepare_ambiguous` states. Notification semantics and exact contract evidence are being verified before any live attempt. Current source work is not deployed.

Next exact step:

- Finish and review the official invoice API correction and a migration-only one-seed verification helper. The helper must use the shared lease, fresh eligibility, existing held seed and exactly one normal generator attempt; it cannot select arbitrary orders, send old emails or replay uncertain phases. Enqueue and manual restoration guards now preserve/block the new preparation phases; 26 state/restoration tests pass. Commit/push reviewed source before any financial canary, verify its actual final document, then require full regression/CI and a new exact immutable managed release. Do not deploy the superseded pending image or claim any of the eleven historical invoices created yet.
