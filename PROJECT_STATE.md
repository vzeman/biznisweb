...
- VEVO GrowthBook Pro rollout has a concrete, read-only preflight, dated baseline, and execution contract on branch `codex/vevo-growthbook` (`2026-08-20`):
  ...
  - Tag Assistant could not attach to `https://www.vevo.sk/` and timed out with zero detected tags for Analytical-only and all-category test consent, a clean session with Analytical consent granted before Preview, and Google's official retry with the URL debug signal disabled, even though the live GTM script was present in the DOM; the no-Analytical control correctly exposed no GrowthBook DOM marker, but the draft container was never evaluated and no Preview event was accepted
  - no ad blocker, Chrome security setting, or extension state was changed during troubleshooting; the supported next gate remains explicit approval before installing or enabling the official Google Tag Assistant extension
  - Next exact step: after explicit user confirmation, install or enable the Google Tag Assistant browser extension and repeat the unpublished Preview checklist; require connected Tag Assistant, no-Analytical zero marker, Analytical-only loader/exposure, stable A/A assignment, consent-withdrawal cleanup, and zero cart/checkout mutation. Do not publish GTM, start an experiment, or change Production allocation
