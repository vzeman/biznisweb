...
- ROY exact zero-revenue gift allowlist is implemented and awaiting production deployment (2026-07-16):
  - the gift exception is now restricted to exact reporting SKUs `R99003` (`Sada nožov Roy 3-dielna Lux`), `11004` (`Roy Hunter Knife 11004`), and `11001` (`Roy Hunter Knife 11001`); it applies only to a positive-quantity item line with exactly `EUR 0` net revenue, while positively priced lines retain their mapped purchase costs of `EUR 16.58`, `EUR 4.17`, and `EUR 4.00` per unit
  - removed the broad knife-label matcher, the old `11004` zero-price exclusion that hid gift rows completely, and the Lux legacy zero-cost fallback that could have become unsafe if its mapped cost disappeared; malformed or duplicate exact-SKU configuration now fails closed
  - pre-deploy hard-gate: compute is ECS/Fargate (`instance-id=N/A`, no fixed task IP), schedule `roy-daily-report-email` is `ENABLED` on `roy-reporting-daily:50`, App Runner service `biznisweb-roy-operations-dashboard` is `RUNNING`, runtime gate path is `/app/scripts/live_dashboard_refresh_gate.sh`, current DNS IPs are `3.120.216.162`, `3.75.104.192`, and `3.126.228.15`, and `/health` returned HTTP `200`; the ecommerce EC2 runtime is outside this deployment scope
  - verification: `192` unit tests, `70` focused reporting-calculation tests, reporting QA smoke, Python compile, ROY JSON parse, `git diff --check`, and a local cached-history replay passed; the replay kept zero gift cost on all three exact SKUs and retained mapped cost on every paid row
  - production expectation from generation `20260716T014642Z`: correcting `261` free `11001` units removes `EUR 1,044.00` of false cost, changes its gross product result from `-EUR 849.05` to `+EUR 194.95`, and raises full company profit from `EUR 20,490.09` to approximately `EUR 21,534.09`; the formerly hidden `11004` gift rows will increase audited item/gift counts without changing revenue or profit
  - implementation branch: `codex/roy-explicit-gift-products`
  - Next exact step: commit and push the branch, merge through PR after CI, deploy the exact merged image with the ROY live-dashboard workflow, require the Fargate localhost/artifact markers before schedule promotion, and verify the regenerated full-history S3 payload plus UI
