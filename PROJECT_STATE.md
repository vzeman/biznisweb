...
- VEVO GrowthBook Production clone is verified complete and still hard-disabled for traffic (`2026-08-23`):
  ...
  - Next exact step: merge schema `9`, then prepare the exact GrowthBook Production A/A start and feature revision `3` publish action for action-time confirmation; after the start, immediately verify live `100%` A/A traffic, frozen `50/50` weights, sticky assignment, accepted collector delivery, unchanged CTA/cart/checkout/orders, and record the activation read-back through Git
