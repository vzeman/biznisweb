...
- ROY exact zero-revenue gift allowlist is merged; the final clean backfill is gated by a new payment-metadata safety fix (2026-07-16):
  ...
  - branch `codex/roy-period-inventory-snapshot-gate` now fetches one ROY inventory snapshot before any period child export, fails closed on an exception or empty snapshot, shares the verified frame through a run-scoped cache, and gives every 7D/30D/90D/full analyzer a deep copy; this removes three redundant 109-page API scans and prevents period-specific inventory drift; `214` full tests, `83` focused tests, `13` inventory-model tests, reporting QA smoke, compile, JSON validation, and `git diff --check` passed
  - Next exact step: commit/push and merge the period-inventory gate, build its exact digest, rerun the ROY full-history deploy, require one `PERIOD_BUNDLE_INVENTORY_SNAPSHOT_READY` plus four cache-reuse markers, zero unresolved non-overridden payment metadata, `3,298` realized orders, valid inventory in all period payloads, expected clean full company profit near `EUR 21,534.06`, and the exact gift/paid SKU splits before final production promotion
