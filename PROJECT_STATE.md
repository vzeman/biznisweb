...
- ROY exact zero-revenue gift allowlist is merged; the final clean backfill is gated by a new payment-metadata safety fix (2026-07-16):
  ...
  - branch `codex/roy-nonrealized-order-2677001207` adds an exact ROY-only missing-metadata non-realized override with a mandatory audit reason; paid status or later-loaded real payment metadata still wins, VEVO is isolated, wildcard/blank configuration fails closed, cache and mixed-enrichment paths remain guarded, and every other unresolved candidate still raises `PaymentMetadataEnrichmentError`; `212` full tests, `81` focused tests, reporting QA smoke, compile, JSON validation, `git diff --check`, and two independent P0/P1 reviews passed
  - Next exact step: commit/push the exact override, merge it through PR, build its exact digest, rerun the ROY full-history deploy, require zero unresolved non-overridden payment metadata plus `3,298` realized orders, verify expected clean full company profit near `EUR 21,534.06`, validate all three gift/paid SKU splits and all period payloads, then record the final production generation and monitor the next scheduled run
