...
- VEVO GrowthBook Pro rollout has a concrete, read-only preflight, dated baseline, and execution contract on branch `codex/vevo-growthbook` (`2026-08-20`):
  ...
  - Next exact step: replace the invalid task-definition path assertion with another exact one-shot Fargate localhost marker gate, then create exactly one `vevo-growthbook-preview-reader` access key and connect only the Preview Athena data source. Production, GTM publishing, Meta, and BiznisWeb remain unchanged
