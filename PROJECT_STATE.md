...
- Monthly combined ROY+VEVO credit-note export automation was implemented locally on `2026-06-17`:
  - scope: one repo-local accounting export run for credit notes plus Money S3 invoice exports, not GraphQL invoices
  ...
  - credit-note output: one PDF file in `data/combined_exports`; no XLSX workbook and no source JSON sidecar are generated by the monthly credit-note run
  - invoice output: one Money S3 XML per shop from BizniWeb admin `/erp/impexp/export/index/invoices/moneys3`; invoices are filtered by issue date (`inv_date_from` / `inv_date_to`)
  - email: SES raw email with the PDF dobropisy attachment and Money S3 invoice XML attachments; default recipient `mil.terem@gmail.com`; sender comes from `CREDITNOTE_EXPORT_EMAIL_FROM` or `REPORT_EMAIL_FROM`
  - schedule metadata: `projects/roy/settings.json` -> `monthly_creditnote_export`, schedule `monthly-creditnote-export`, cron `cron(0 6 14 * ? *)`, timezone `Europe/Bratislava`, task family `monthly-creditnote-export`, projects `roy,vevo`, invoice format `moneys3`, invoice date field `inv_date`
  ...
  - local verification:
    - `python -m py_compile creditnote_export.py money_s3_invoice_export.py monthly_creditnote_export_runner.py`
    ...
    - `python monthly_creditnote_export_runner.py --reference-date 2026-06-14 --skip-email --output-tag local_accounting_default_smoke` returned `exported_rows=39`, ROY `14`, VEVO `25`, generated a valid `%PDF-` file, generated Money S3 XML invoice files for ROY `384` and VEVO `454` invoices by invoice issue date, and did not create `.xlsx` or `_source.json` artifacts for that tag
    - tested Money S3 invoice export directly with `run_money_s3_invoice_export(..., output_tag='local_money_s3_only_smoke')`, returning ROY `384`, VEVO `454`, total `838`, with XML files starting `<MoneyData`
    ...
    - `git diff --check`
  - local generated artifacts: `data/combined_exports/dobropisy_actual_roy_vevo_2026-05_created_local_accounting_default_smoke.pdf`, `data/combined_exports/faktury_money_s3_roy_2026-05_issued_local_accounting_default_smoke.xml`, `data/combined_exports/faktury_money_s3_vevo_2026-05_issued_local_accounting_default_smoke.xml` (ignored export output)
