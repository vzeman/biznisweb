...
- VEVO GrowthBook Pro rollout has a concrete, read-only preflight, dated baseline, and execution contract on branch `codex/vevo-growthbook` (`2026-08-20`):
  ...
  - no IAM user/access key, committed GrowthBook client key, DNS record, GTM tag, BiznisWeb script, Meta change, AWS stack, or AWS resource was created; the local AWS CLI has no active credentials
  ...
  - Next exact step: resolve and record the serverless host-verification policy mismatch (exact runtime identity/IP/service/path plus localhost marker strategy) before any Preview stack, Athena identity/data source, GTM entry, collector event, or SDK connection test; then install only the staging Preview tag and run consent/rollback QA before A/A can start
