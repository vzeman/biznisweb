...
- VEVO GrowthBook Pro rollout has a concrete, read-only preflight, dated baseline, and execution contract on branch `codex/vevo-growthbook` (`2026-08-20`):
  ...
  - Next exact step: audit same-population exposure/outcome joins and the existing VEVO Meta campaign/adset/ad dimensions without changing Meta delivery; then wait for and verify the first natural reconciliation at `2026-08-22 03:30 Europe/Bratislava` before publishing only the measured A/A instrumentation. Do not start CTA A/B or change Production allocation
