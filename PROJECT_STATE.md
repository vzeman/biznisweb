...
- ROY blocking dashboard maintenance mode is implementation-complete and release-ready (2026-07-22):
  ...
  - Next exact step: merge the maintenance PR, build the exact merge image, run one hard-gated ROY bootstrap deploy with `skip_artifact_refresh=true`, then run a normal protected deploy and require S3 active, current App Runner active, Fargate localhost active, new App Runner active/`423`, inactive, restock restore, and final deploy markers before declaring production complete
