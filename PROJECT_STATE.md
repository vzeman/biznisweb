...
- VEVO GrowthBook Production clone is verified complete and still hard-disabled for traffic (`2026-08-23`):
  ...
  - mobile viewport, explicit zero-assignment/zero-collector-request read-back, owned-storage cleanup, and complete GA4/Meta consent behavior are still unverified; the activation manifest records each as `false`, keeps GTM unpublished and traffic at `0%`, and cannot advance to controlled activation
  - Chrome and the native host are running and the native-host manifest is valid, but the browser extension repeatedly lost communication while listing/creating a mobile QA tab; the Windows fallback stopped before resizing because it could not safely prove the active Chrome URL, so no window or security setting was changed
  ...
  - Next exact step: merge the schema-3 fail-closed desktop observation, restore reliable Chrome browser-plugin communication, and complete the five pending Tag Assistant checks on desktop/mobile; do not publish GTM, start the A/A, or raise Production allocation above `0%`
