...
- VEVO GrowthBook Pro rollout has a concrete, read-only preflight, dated baseline, and execution contract on branch `codex/vevo-growthbook` (`2026-08-20`):
  ...
  - PR run `32396581447` independently passed `security-baseline`; its Linux Docker task emitted `COLLECTOR_LOCALHOST_HEALTH_OK:preview:git-ci-host-gate` and `COLLECTOR_LOCALHOST_MARKER_OK:/app:git-ci-host-gate`. Env and observability passed; the narrow Gitleaks false-positive fix is the only follow-up before rerunning CI
  ...
  - Next exact step: merge the reviewed Fargate collector through PR, require CI's Docker localhost gate, then dispatch the protected workflow first with `activate_route=false`; record the exact Fargate task ID/private IP/service/path/marker evidence before a second `activate_route=true` dispatch can expose the collector. Only after that may the Athena Preview connection and staging-only GTM tag be configured for consent/rollback QA
