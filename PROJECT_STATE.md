...
- ROY exact zero-revenue gift allowlist is merged; the final clean backfill is gated by a new payment-metadata safety fix (2026-07-16):
  ...
  - branch `codex/roy-payment-metadata-hard-gate` now retries only unresolved candidate orders three times, preserves successful metadata, invalidates only unsafe candidate cache rows, raises a dedicated exception with exact order numbers, propagates it through every fetch fallback, and atomically blocks status filtering; `208` full tests, `77` focused tests, reporting QA smoke, compile, `git diff --check`, and independent P0/P1 review passed
  - Next exact step: merge the payment-metadata hard gate through PR, build its exact digest, rerun the ROY full-history deploy, require zero unresolved payment metadata plus `3,298` realized orders, verify expected clean full company profit near `EUR 21,534.06`, validate all three gift/paid SKU splits and all period payloads, then record the final production generation and monitor the next scheduled run
