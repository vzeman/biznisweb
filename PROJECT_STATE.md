...
- Completed the invoice transport correction and migration-only one-seed helper. Preparation records its claimed preinvoice identity durably; conflicting readback remains uncertain across later runs. A final invoice that disappears during recovery cannot mark its operation complete. Native response IDs are not assumed to be invoice IDs. The served native parser proves boolean `true` and literal string `"true"` success flags; other truthy values remain unconfirmed. Optional email uses the freshly verified order ID, and historical holds remain silent.
...
- Final complete build regression passes 572 tests with exit zero; reporting smoke, focused Ruff and whitespace checks pass. Independent review passed 120 invoice/state/helper/scanner tests and found no remaining concrete source blocker. The helper's 21 tests cover preview, lease and batch gates, exactly one attempt, missing-preinvoice preparation, lost responses, fresh document numbering and held emails. No live financial canary has run; actual document creation remains unverified. All finite local tests and inspectors ended, with no local application service or tunnel started.
...
- Bounded repeated live reads established a stable API convention, not transient count skew: a full terminal page can advertise a next page whose cursor equals `totalRecords`; a following empty read proves exhaustion. Both overlapping and single-row probes reproduced it. The scanner correction accepts only this full-terminal case with exact cursor and derives logical remaining rows from the validated offset/count. Short/empty advertised continuation, false-before-end, count/row mismatch and invalid cursor remain failures. Eight direct scanner tests and the independent 154-test focused regression passed.
- ROY also completed a full 226-page scan and a later two-page incremental scan, but each live run had six definite creation rejections, zero created/sent/ambiguous invoices and a released lease. Its six holds remain `create_failed`; VEVO's five holds remain pending. Failure metrics reached CloudWatch and both run-failed alarms entered ALARM; this proves monitoring, not successful invoicing.
- The stricter invoice rewrite retained an unverified legacy first request and removed its fallback sequence. Actual served administration code instead finalizes an existing preinvoice using its internal identity. All six ROY and four VEVO historical candidates have one preinvoice; the last VEVO candidate has none. The native missing-preinvoice form also carries note/status/date fields and defaults its email checkbox on, so copying that form is not a safe generic API implementation.
- Both live schemas expose GraphQL `preinvoiceOrder` and `finalizeInvoice`, but real `Preinvoice.preinvoice_num` and nested `order` reads fail internally on both shops; neither exposes a direct `getPreinvoice` lookup. Do not guess a number or select those broken fields in a mutation response. The verified correction prepares a missing preinvoice through `preinvoiceOrder { id }`, rereads its unique association on the order, then performs the single native GET finalization using that preinvoice's internal ID. The exact native implementation is identical on both shops. Preparation sends explicit `NONE` conditions for customer, admin and salesperson notifications; an empty notification list has no proven suppression contract. Existing preinvoices need no preparation request.
- Journal phases `preparing`/`prepare_ambiguous` preserve uncertainty without replay. Finalization and optional email each have zero transport retries; the verified email route uses the order ID. Sanitized private contract evidence is under `data/roy/order-automation/web-contract-20260909.json`, SHA-256 `5ff5bb99520ded19895bcc964bb0d11057ea0b81780ddb4a06d6221a9d25564d`; no customer data, credentials or raw form values are committed. The read-only inspector closed both HTTP sessions and its finite Python process; independent checks found no process or listener left behind. Current source work is not deployed.

Next exact step:

- Finish and review the verified invoice transport correction and the migration-only one-seed helper. Require full regression, clean pushed source, PR checks and a new exact immutable managed release. The old image cannot preserve the new preparation phases, so do not run a financial canary while old writers remain scheduled: first require all three candidate localhost gates, managed drain and new-image promotion. Then inspect natural runs and use the helper only for a remaining unambiguous held seed while the shared lease is free. The helper performs exactly one normal generator attempt with fresh eligibility, no arbitrary-order option and no historical email. Verify actual final documents and all eleven private seed outcomes. Do not deploy the superseded pending image or claim any historical invoice created yet.
- The sanitized web/API contract artifact has also been published with encryption, create-only protection and exact hash readback to private S3 `data/roy/order-automation/audits/2026-09-09/web-contract-20260909.json`; its hash above binds the local generated copy to runtime evidence.
- The separately published native response-parser evidence at `data/roy/order-automation/audits/2026-09-09/native-response-parser-20260909.json` has SHA-256 `03135eb0705a1132fd93bf654c27628872f87bdb66aca4f3abc7987405500b12`, with the same encrypted create-only/readback protections.
- Follow-up option for the separately pinned daily creditnote guards: both exact old images already support `REPORT_SKIP_CREDITNOTE_STORNO_GUARD=true`. A reviewed Scheduler environment override can disable that writer while retaining both report task definitions/images, including VEVO's fixed `:33`. A separate committed guard runner, task families, host gates, shared journal access and monitoring must exist first; the guard module currently has no CLI. Preserve cache freshness because the old inline guard signals cache clearing after changes. Record the source-Scheduler configuration transition outside any A/A evidence capture. No report override or guard infrastructure change has been made; fresh report-host identity is still required before implementation.
...
- Extracted the existing bounded unique-ID scanner into `order_inventory.py`, preserving invoice full/changed shared page/time budgets, read pacing and lease heartbeat. Unpaid discovery now reads one unfiltered inventory with its own payment-element fields and filters configured status IDs locally. Its default and ROY configured page limit are 5,000, with two-second pacing, four bounded read attempts and the shared twenty-minute deadline. Incomplete discovery stops before the mutation lease; fresh and final payment/status guards remain unchanged.
- Independent review caught a host-gate evidence gap: requesting full discovery alone was sufficient for the marker. The gate now requires the actual runner's `invoice_scan_all_ages=true` before its localhost marker, with missing/false evidence regressions. Legacy pin behavior remains compatible. Both CI regression commands now include the shared scanner tests.
...
- Full build regression passed 527 tests; reporting smoke, focused Ruff and whitespace checks passed. Independent review separately passed 181 scanner/invoice/status/deployment tests and found no remaining concrete defect. Regressions cover offset deletion, one-page cross-status movement, timestamp ties, metadata failures, partial responses, retry pacing, shared budgets and no writes after an incomplete inventory. All finite local test processes ended; no local service or tunnel was started.
- Fresh post-host-gate admin UI confirms CASE-A retains its already-existing final invoice and CASE-B remains shipped with its original invoice. The first natural upgraded VEVO task `0196a28edb25493ba055f52a3b736c15`, IP `172.31.41.230`, service `vevo-invoice-daily:5`, started with the exact image and a renewing shared lease. Its five historical records were still pending before discovery finished; do not claim final invoice creation yet.
...
- Push the reviewed complete-discovery follow-up and open its PR; require exact-head Windows/Linux CI before merging and an exact immutable build before the second managed release. Finish and independently verify the eleven historical invoices through the current live release. Final live cancellation baseline must use the complete-discovery release. Keep the separate FLOX and daily-report guard limitations explicit.
...
- The new ROY candidate passed: task `24395a2c00c74d8cabc680566186219c`, IP `172.31.33.141`, service `roy-invoice-daily:5`, exact digest above, `/app`, actual curl-localhost marker and independently verified STOPPED/exit zero. It matched six candidates with no created/sent documents or failure counters. VEVO candidate `9ec1285e01d44f81a5e73d655044f763`, IP `172.31.13.154`, service `vevo-invoice-daily:5`, has started preparation; its result and cancellation/promotion gates remain pending.
...
- A bounded, fully paginated CloudWatch check around CASE-B's earlier short-age cancellation found no matching order or status-write signal in the four relevant invoice/report/cancellation groups. ROY invoice events had ended several minutes before that action; the nightly cancellation task had no events in the window. No further relevantly named Lambda/log group or ROY EventBridge rule was found. This does not attribute the action or exclude an unlogged, differently named, external or admin writer; present inventory is not proof of historic configuration.
- Minimal private evidence for that negative attribution check was published with encryption, create-only protection and exact readback under `data/roy/order-automation/audits/2026-09-09/early-cancellation-log-audit-*.json`, SHA-256 `436b77454f1c06817d73812c63072a9850a9063c91c25d16b0314c747658d2ea`. It contains the private window and aggregate query results, not raw logs or secrets.
...
- ROY candidate task `43235383df0f40a388691a91b173169c`, IP `172.31.36.32`, service `roy-invoice-daily:4`, `/app`, stopped with exit zero and an actual curl-localhost marker. It found six invoice candidates, created/sent none, and reported no application failure. VEVO candidate `861ce30becb543f88548e79ca3e46202`, IP `172.31.24.63`, service `vevo-invoice-daily:4`, `/app`, stopped with exit one on a repeated order during its full offset scan. No cancellation candidate or business write ran in this deployment.
- A bounded read-only API probe confirms supported unique `order_id` ASC/DESC ordering and complete unfiltered inventories of 4,778 ROY / 8,869 VEVO orders. The API cursor is an offset; `newer_from` is inclusive and cannot by itself resolve timestamp ties. A small repeated VEVO tail sample did not reproduce the failure, so its exact trigger remains unproven. Do not attribute it definitively to status movement or ties.
...
- The verifier's first Windows CI exposed a test-only path alias mismatch: its expected temporary path was not resolved, unlike the helper's canonical output path. `acfde3e3` normalizes the test expectation; all six checks now pass on that head and the runtime path containment check remains unchanged. Draft PR #542 tracks verification and the pending pagination correction.
- The new discovery source uses bounded unique-ID overlap pagination and validates response metadata before claiming completion. Independent synthetic stress across 500 deletion/append/timestamp-tie cases found no successful scan that omitted a surviving original ID or included a later ID; severe churn fails at a finite repair bound. A shared twenty-minute discovery deadline covers full/incremental scans, anchor/repair reads and retry waits. Fifteen new pagination regressions and strict fresh mutation checks pass independent review.
- Combined local build regression passes all 512 tests, including 25 independent discovery/backfill verification tests. Reporting smoke, focused Ruff and whitespace checks pass. The full-discovery helper is committed/pushed in `376593d3`; it requires clean, tracked, exactly pushed source, emits only aggregate progress and optionally persists immutable encrypted evidence. All six exact-head PR checks, including Windows/Linux regression, pass on pagination correction `abdc41f15945204966489dfd142af90b33f4df79`.
- Actual read-only discovery from that clean pushed source passed both inventories: ROY 4,778 orders / 166 pages / all six historical candidates still eligible; VEVO 8,869 / 307 / all five still eligible. Both completed with zero failed orders/global issues and no invoice, email, status or journal writes. Each private report was published immutably with encryption and verified by readback under `data/<project>/order-automation/verification/invoice-discovery-*.json`. SHA-256: ROY `525424748602aa070d462ec3f22146edb823f0e80f77dc1f2b82775949b49ec2`, VEVO `648fa31d2b17caa97acf0828e4f9bc043003b1de2c1b481c73f6c661d23ba6ff`. Both finite processes exited zero; their recorded PIDs are gone and no listening ports remain.
- Read-only ninety-day changed-order counts are 1,691 ROY / 1,071 VEVO. Increased both invoice read pacing settings from one to two seconds: VEVO's approximately 343 full-plus-incremental pages have an estimated eleven-and-a-half-minute minimum, leaving room within the shared discovery budget and separate thirty-minute candidate host bound. This estimate is not live completion evidence. README and agent guidance now point to all-age previews and the managed operations runbook rather than an obsolete seven-day invoice example.
...
- The rejected candidate uses status-filtered, nonunique purchase-date order with offset pagination. Its fail-closed duplicate guard correctly rejected a repeated VEVO row. Replacement source, regressions and both complete real inventories now pass; newly built candidate hosts still must pass before production promotion.
...
- Resolved a live VEVO creditnote normalization blocker before deployment: the complete 313-document list contains 13 opened, zero-value entries without final document numbers. They remain attributed to their own orders with unknown credit coverage, preventing false refund/recovery decisions without rejecting the other documents. Corrected read-only normalization returns 304 orders, 300 numbered documents and 13 unnumbered entries in EUR/CZK. Creditnote-context failures now retain status review/failure metrics while independently verified invoice creation continues; loss of the safety lease still stops all mutations. Numeric zero parsing and these failure boundaries have synthetic regressions; 129 combined focused tests, Ruff and diff checks pass. No financial mutation was made during this investigation. Next: include this fix in the candidate image and complete the deployment gates.
- The immutable-image preflight stopped before AWS writes because legacy invoice runtime secrets contain an empty `REPORT_S3_BUCKET`. Read-only inspection of the actual reporting schedules verified `roy-reporting-daily:71` and `vevo-reporting-daily:33` use explicit bucket `biznisweb-reporting-artifacts-919341186960-eu-central-1`, with prefixes `daily-reports/roy-sk` and `daily-reports/vevo`. Current-account ownership, region and all four public-access blocks were verified. Recorded those non-secret project destinations and added one reusable storage resolver for the automation journal, historical seed and strict creditnote audit. Empty runtime values can use the explicit project configuration; missing configuration cannot invent a bucket. Both real secret bundles now resolve to the verified destinations in read-only checks. All 97 focused storage/invoice/creditnote/seed tests, Ruff and diff checks pass. Next: deployment must compare configuration with the live reporting target, inject canonical storage into new candidates without conflicting secrets, then resume its gates; no AWS or financial mutation was performed in this investigation.
...
- Final combined local regression gate passes all 464 tests; focused changed-file Ruff and whitespace checks pass. CI now includes the guarded historical fulfillment repair. Documented optional storage overrides and added creditnote review/partial-credit health metrics. All agent edits are complete; next is immutable pinning of existing invoice behavior, private historical seeding, exact-head CI/merge/build and managed candidate deployment. No production promotion or financial mutation has occurred yet.
- Pre-merge delivery milestone: all six CI checks passed on `6b2bb8f2`. Existing invoice behavior is now pinned to verified digest `3279c0e6...0162` in both `:3` task definitions and all four invoice schedules, with cadence preserved. Actual dry-run host/curl markers passed on ROY task `2a3b063c661e4422922fff73c174f674`, IP `172.31.22.25`, and VEVO task `3ce90496a5c042018fd7b8e9b837a013`, IP `172.31.24.102`, `/app`; both stopped with exit zero. Exact snapshots and markers are in private S3 deployment evidence. Historical seeding readback confirms six ROY and five VEVO pending records with email holds and no retained lease. CASE-B passed fresh full-payment/creditnote checks and was silently restored to its independently documented shipped state with verified API readback. No historical invoice has been created by the new runner yet. Next: merge after final documentation CI, build exact image, run the three managed candidate gates and promote schedules.
- Fresh admin UI/history confirms CASE-B is shipped and the repair emitted no customer email. Final workflow review added image-build triggering for standalone shared status-safety edits and PR tests for standalone migration/repair helper edits, so later narrow changes cannot bypass their validation/build path.
- Closed a final deployment race in `810a9410`: candidate tests can overlap later legacy launches, so promotion now pauses the five schedules, waits for pending/running/stopping jobs and two continuously quiet minutes, and only then enables the verified new definitions. Natural jobs are never terminated. Partial-promotion rollback drains the candidate generation before resuming old jobs; concurrent drift remains protected. All 29 deployment tests and independent review pass. Increased the managed timeout to cover bounded hosts and rollback drains. The reporting smoke gate also passes; final combined regression and exact-head CI precede merge.
- Final combined application/deployment regression passes all 472 tests after auth-log sanitization and drain integration; reporting smoke, focused Ruff and whitespace checks pass. No local application service was started. Merge remains gated by final exact-head CI.
- Removed legacy invoice-authentication diagnostics that exposed full or partial authentication tokens, session cookies, response headers/bodies, token-bearing URLs and raw exception messages. Authentication logs now contain only safe phase/status/type information; DEBUG no longer writes raw login/dashboard response files. Request flow, token extraction and authentication outcomes are unchanged. Six isolated synthetic log-capture probes and all 57 invoice/state regressions pass, with Ruff and diff checks clean. Next: include this final security fix in exact-head CI and the candidate image; no production credentials or financial state were changed.
...
- Shared status safety now separates receipt settlement, fulfillment and creditnote coverage, suppresses corrective emails, and independently verifies single-attempt writes; 33 focused tests pass. Read-only runtime queries confirmed that explicit null document/payment collections mean no entries and shipment lists can contain empty carrier placeholders. Neither an invoice nor an empty shipment collection proves dispatch history. Uncertain writes and repeated regressions require review; a later freshly proven full-creditnote cancellation remains possible after a verified repair.
- Unpaid cancellation and the creditnote guard now use the shared per-shop lease, durable mutation journal and final fresh evidence checks. Incomplete scans fail before writes; bank receipts protect paid orders without requiring a final invoice; only exact full creditnote coverage permits whole-order Storno. Pre-change fulfillment audit is durable before that write, corrective emails are suppressed, and unresolved writes/storage failures cannot report success. All 66 focused status/cancellation/creditnote tests pass, including late payments/creditnotes, partial refunds, uncertain responses, repeated cursors and distinct live/dry-run health metrics. Source is committed for CI review; production has not been promoted.
- Runtime payment verification exposed a legitimate nullable receipt date: the [official receipt schema](https://www.biznisweb.sk/api/docs/receipt.doc.html) permits null, and the [invoice paid flag](https://www.biznisweb.sk/api/docs/invoice.doc.html) records full settlement. An exactly paid invoice with consistent full receipts now remains confirmed when only that optional date is null; missing fields, malformed dates, partial receipts, reversals and conflicts remain protected. A fresh read-only CASE-B query confirms this outcome. Also removed the incorrect assumption that `is_net_price=true` makes the order total net: live VAT arithmetic proves the flag can be inverted, matching the existing reporting resolver. Creditnote coverage now requires gross credit amounts to match the payable total with a consistent VAT summary, or equal net/gross amounts for tax-exempt records. All 116 combined invoice/status/guard tests and focused Ruff checks pass; no live financial mutation was performed for this validation.
- Nightly cancellation and creditnote writers now wait up to 15 minutes for an invoice run to release the shared lease, checking every 30 seconds. Only the explicit busy-before-acquisition condition is retried; storage uncertainty, lost ownership and failures after entry remain terminal and never replay the mutation body. Current order/payment/creditnote evidence is rechecked after acquisition. All 76 focused status/cancellation/guard tests pass, including bounded contention and no-replay regressions. Schedule cadence is unchanged; an exhausted wait still reports failure for monitoring.
...
- The separately scheduled daily reporting images are immutable and still contain the older creditnote guard: ROY `roy-reporting-daily:71`, VEVO `vevo-reporting-daily:33`. The three-service automation release does not promote those report tasks. VEVO `:33` is also the explicitly fixed A/A source in the activation/workspace contracts, so an incidental report image upgrade would violate that separate source contract. Guard code/tests are prepared, but daily guard production deployment requires a separate reviewed reporting/source transition; do not report those daily guard fixes as live or modify the frozen source to complete this release.
...
- Replaced the overview's silent `orders.slice(0, 24)` truncation with pagination in both order tables using the complete existing snapshot, default 10 rows with 25/50 options, preserving the selected page during refresh and clamping it when the list shrinks. Picking actions retain the complete order list.
...
- PR `#537` passed all four exact-head CI checks and merged as `e6da0550710309b3e1af512eeef84ddc51354357` at `2026-09-09T03:14:36Z`. Full generated-dashboard JavaScript syntax also passed Node validation.
- ECR build `34306363312` succeeded, including the reporting QA smoke and 309 regression tests. Verified the exact merge tag and `latest` both resolve to `sha256:06edde2801ddef74c7e4c84f154233575fa8f5715c7724da36c662245f6602ad`, main still matched the reviewed merge, and no other dashboard deploy was active before dispatching protected ROY-only deploy `34306554292` with `skip_artifact_refresh=true`.
- Candidate host gate passed: Fargate task `efaf13b9f4eb44cfaf97364677dcbacd`, private IP `172.31.9.15`, service `roy-daily-report-email`, task definition `roy-reporting-daily:76`, runtime `/app`, exact digest above. CloudWatch confirms `LOCALHOST_ROY_ORDER_PAGINATION_OK:controls=4:truncation=false`, `LOCALHOST_LIVE_DASHBOARD_OK:roy:periods=7d,30d,90d,full` and localhost JSON `LIVE_ARTIFACT_MARKER_OK` for ROY in skip-refresh mode. Task independently confirmed STOPPED, exit `0`.
- Deploy `34306554292` completed successfully. App Runner operation `a3ad9e8c25064fe4ad4ea8f199ebcd82` succeeded; service is RUNNING on the exact candidate digest. Final markers include live operations/PDF, maintenance write-block/inactive, restored restock sentinel and `APP_RUNNER_DEPLOY_OK`. No daily artifacts were regenerated by this UI-only deploy.
- Authenticated Chrome verification after the host gate confirmed 19 live orders across 10 + 9 rows, 19 unique order IDs without gaps/duplicates, previous/next controls including bottom navigation, shared page state between views, and both 25/50 selections showing all 19 with Next disabled. The PDF action consistently retains all 8 unprinted orders across pages; no print/order mutation was triggered by browser testing. Visual screenshot inspection passed and browser warning/error logs were empty.

Known issues:
- No blocker remains for this change. No local application service, worker or tunnel was started or left running; finite tests/watch commands completed, and the isolated remote verification task stopped successfully. Production App Runner intentionally remains running.

Next exact step:
- Keep normal ROY operations running; investigate only if pagination or live refresh regresses. Final verification handoff must remain committed/pushed and merged via a documentation PR.
