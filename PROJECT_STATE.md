...
- VEVO GrowthBook Production clone is verified complete and still hard-disabled for traffic (`2026-08-23`):
  ...
  - schema-`8` records the post-publish public read-back and keeps GrowthBook start closed while the protected read-only collector workflow checks the frozen `2026-08-24T14:34:30Z..2026-08-24T14:38:00Z` window for zero API requests and zero accepted receipts
  - Next exact step: merge schema `8`, dispatch the protected post-publish zero-collector workflow on exact main, independently hash and record its sanitized artifact as schema `9`, and only then prepare the final GrowthBook Production A/A start action
