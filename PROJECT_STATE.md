...
- VEVO GrowthBook Pro rollout has a concrete, read-only preflight, dated baseline, and execution contract on branch `codex/vevo-growthbook` (`2026-08-20`):
  ...
  - Next exact step: reconcile the controlled accepted exposure through raw S3, curated facts, Athena, and the GrowthBook assignment query; compare collector/curated/GrowthBook device counts, then add a reviewed recurring reconciliation schedule before collecting a real A/A population. Do not publish GTM, start CTA A/B, or change Production allocation
