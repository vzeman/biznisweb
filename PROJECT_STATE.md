...
- VEVO GrowthBook Production clone is verified complete and still hard-disabled for traffic (`2026-08-23`):
  ...
  - schema-`7` records the consent read-back and authorizes only a zero-allocation GTM publish; GrowthBook start and feature revision publish remain closed until a separate post-publish read-back is merged
  - Next exact step: merge schema `7`, publish only GTM workspace `17` from live version `14` while GrowthBook live revision `2` keeps Production disabled, verify the new live version plus zero Production exposures, and record that read-back through Git before any GrowthBook start
