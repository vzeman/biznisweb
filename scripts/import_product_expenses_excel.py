...
def main() -> int:
    parser = argparse.ArgumentParser(description="Rebuild product_expenses.json from reporting Excel workbook.")
    ...
    parser.add_argument(
        "--keep-json",
        default="",
        help="Optional existing JSON path whose unmanaged keys should be preserved",
    )
    args = parser.parse_args()

    xlsx_path = Path(args.xlsx)
    output_path = Path(args.output)
    keep_json_path = Path(args.keep_json) if args.keep_json else None

    _, rows = load_rows(xlsx_path, args.sheet)
    mapping, unresolved = build_mapping(rows)

    if keep_json_path and keep_json_path.exists():
        with keep_json_path.open("r", encoding="utf-8") as handle:
