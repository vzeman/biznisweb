...
def current_main(commit):
    require(os.environ.get("GITHUB_REF") == "refs/heads/main" and os.environ.get("GITHUB_SHA") == commit,
...
class Deployment:
    ...
    def snapshot_protected(self):
        schedules = {name: self.binding.schedule_snapshot(self.schedule(name)) for name in sorted(PROTECTED)}
    ...
    def tasks(self):
        ...
        for desired in ("RUNNING", "STOPPED"):
            token, seen = None, set()
            for _ in range(100):
                request = {"cluster": CLUSTER, "family": "vevo-reporting-daily", "desiredStatus": desired, "maxResults": 100}
                if token:
    ...
    def drain(self):
        ...
        while self.clock() < deadline:
            ...
            quiet = None if unfinished else quiet or self.clock()
    ...
    def cleanup_role(self):
        if not self.role_created:
            return
        self.cleanup_task()
        name = "VevoReportProbe-" + self.release_id
        role = self.iam.get_role(RoleName=name)["Role"]
        require(role["Arn"] == f"arn:aws:iam::{ACCOUNT}:role/{name}"
                and {r["Key"]: r["Value"] for r in role.get("Tags", [])}.get("ManagedReportProbe") == self.release_id
                and self.iam.list_attached_role_policies(RoleName=name)["AttachedPolicies"] == []
                and self.iam.list_role_policies(RoleName=name)["PolicyNames"] == ["IsolatedProbe"]
                and self.iam.get_role_policy(RoleName=name, PolicyName="IsolatedProbe")["PolicyDocument"] == diagnostic_policy(self.release_id),
                "report-probe-role-cleanup-ownership-invalid")
    ...
    def run(self):
        ...
        try:
            ...
            require(len(images) == 1, "report-exact-image-missing")
            image = f"{ACCOUNT}.dkr.ecr.{REGION}.amazonaws.com/vevo-reporting@{images[0]['imageDigest']}"
            ...
            record = self.binding.build_promotion_record(self.previous, self.schedule(), self.production_definition,
                ...
                build_run_id=str(builds[0]["id"]), protected_sha256=self.binding.sha256(self.protected), candidate_proof=proof,
    ...
    with ExitStack() as stack:
        ...
        class Session:
            def client(self, name):
                ...
                return clients[name]
        Deployment(Session(), args.commit, args.readiness_key, args.readiness_sha256).run()
