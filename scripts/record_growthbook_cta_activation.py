...
try:
    ...
    from evaluate_growthbook_cta import (  # type: ignore
        CtaEvaluationError,
        validate_lifecycle_manifest,
    )
...
EXPECTED_STATIC_HASHES = {
    ...
    "decision_contract": "62d9eb905a05b6273a7395905bc73f815e130155af1a32d896195facd442a07a",
...
def _parse_utc(value: Any, field: str) -> datetime:
    ...
    _require(
        parsed.isoformat().replace("+00:00", "Z") == value, f"{field} is not canonical"
    )
    return parsed
...
def validate_manifest(manifest: Mapping[str, Any]) -> None:
    ...
    _require(
        root["activation_type"] == "vevo_growthbook_product_cta",
        "CTA activation type drift",
    )
    _require(
    ...
    _require(
        root["runbook"] == "projects/vevo/GROWTHBOOK_CTA_ACTIVATION_RUNBOOK.md",
        "CTA activation runbook drift",
    )
    _require(
    ...
    for name, expected_path in EXPECTED_PATHS.items():
        binding = _exact_object(
            bindings[name], {"path", "sha256"}, f"source_bindings.{name}"
        )
        _require(binding["path"] == expected_path, f"{name} path drift")
        _validate_hash(
            binding["sha256"],
            f"{name} SHA-256",
            nullable=name not in EXPECTED_STATIC_HASHES,
        )
        if name in EXPECTED_STATIC_HASHES:
            _require(
                binding["sha256"] == EXPECTED_STATIC_HASHES[name],
    ...
    _require(
        launch["variation_weights"] == {"control": 0.5, "brand_contrast": 0.5},
        "CTA weights drift",
    )
    ...
    _require(
        launch["checkpoint_policy"] == "outcome_blind_eligible_device_count_only",
        "CTA checkpoint policy drift",
    )
    _require(launch["gtm_container_version_id"] == "15", "CTA GTM version drift")
    _require(
        launch["growthbook_experiment_id"] == "exp_19g6mmt1qxzrp",
        "CTA GrowthBook ID drift",
    )
    _require(launch["data_source_id"] == "ds_19g6mmt5stlp6", "CTA data source drift")
    _require(
        launch["production_registry_experiments"] == [root["experiment_id"]],
        "CTA-only registry contract drift",
    )
    ...
    requirements = _exact_object(
        root["runtime_readiness_requirements"],
        {
            ...
            "service",
            "runtime_path",
            "localhost_marker_required",
            "target_health",
            "cta_only_production_registry_required",
            "zero_cta_events_before_start_required",
            "gtm_unprocessed_changes_required",
            "aa_production_allocation_percent_required",
    ...
    _require(
        readback["observation_path"]
        == "projects/vevo/growthbook_cta_activation_observation.json",
        "CTA start observation path drift",
    )
    ...
    if root["status"] == WAITING:
        for name in dynamic_hashes:
            _require(
                bindings[name]["sha256"] is None,
                f"waiting CTA manifest binds {name} too early",
            )
        _require(
            all(
                runtime_binding[field] is None
                for field in ("observation_sha256", "workflow_run_id", "main_commit")
            ),
            "waiting CTA runtime binding is populated",
        )
        _require(
            launch["target_total_sample"] is None, "waiting CTA target is populated"
        )
        _require(
            readback
            == {
        ...
        _require(
            boundaries["manual_growthbook_start_allowed"] is False,
            "waiting CTA manual gate is open",
        )
        _require(
            root["next_gate"]
            == "after_aa_pass_stop_sample_freeze_lifecycle_and_cta_only_host_gate_open_manual_start_review",
    ...
    _require(
        isinstance(runtime_binding["workflow_run_id"], str)
        and RUN_RE.fullmatch(runtime_binding["workflow_run_id"]) is not None,
        "runtime workflow run ID is invalid",
    )
    _require(
        isinstance(runtime_binding["main_commit"], str)
        and COMMIT_RE.fullmatch(runtime_binding["main_commit"]) is not None,
        "runtime main commit is invalid",
    )
    ...
    if root["status"] == REVIEW_OPEN:
        _require(
            boundaries["manual_growthbook_start_allowed"] is True,
            "reviewed CTA manual gate is closed",
        )
        _require(
            readback["status"] == "not_recorded",
            "CTA start was recorded before manual start",
        )
        _require(
            all(
            ...
            "CTA readback fields populated before start",
        )
        _require(
            root["next_gate"]
            == "manually_start_only_exact_cta_then_record_canonical_readback",
            "CTA reviewed next gate drift",
        )
        return

    _require(
        boundaries["manual_growthbook_start_allowed"] is False,
        "running CTA manual start gate remains open",
    )
    _require(
        readback["status"] == "verified_running_production_cta",
        "CTA running readback status drift",
    )
    ...
    started = _parse_utc(
        readback["assignment_started_at_utc"],
        "start_readback.assignment_started_at_utc",
    )
...
def validate_runtime_observation(
    ...
    _require(
        root["evidence_type"] == "vevo_growthbook_cta_runtime_readiness",
        "CTA runtime evidence type drift",
    )
    _require(
        root["experiment_id"] == manifest["experiment_id"],
        "CTA runtime experiment drift",
    )
    _parse_utc(root["observed_at_utc"], "runtime observed_at_utc")
    workflow = _exact_object(
        root["workflow"], {"run_id", "main_commit", "conclusion"}, "runtime workflow"
    )
    _require(
        isinstance(workflow["run_id"], str)
        and RUN_RE.fullmatch(workflow["run_id"]) is not None,
        "runtime workflow run ID is invalid",
    )
    _require(
        isinstance(workflow["main_commit"], str)
        and COMMIT_RE.fullmatch(workflow["main_commit"]) is not None,
        "runtime workflow main commit is invalid",
    )
    _require(
        workflow["conclusion"] == "success", "CTA runtime workflow did not succeed"
    )
    ...
    try:
        ...
        raise CtaActivationRecordingError(
            "CTA host-gate private IP is invalid"
        ) from exc
    _require(
        runtime["service"] == "vevo-growthbook-collector-production",
        "CTA runtime service drift",
    )
    _require(runtime["runtime_path"] == "/app", "CTA runtime path drift")
    _require(
        isinstance(runtime["task_definition"], str)
        and TASK_RE.fullmatch(runtime["task_definition"]) is not None,
        "CTA task definition is invalid",
    )
    _require(
        isinstance(runtime["host_gate_task_id"], str)
        and TASK_ID_RE.fullmatch(runtime["host_gate_task_id"]) is not None,
        "CTA host-gate task ID is invalid",
    )
    _require(
        isinstance(runtime["image_digest"], str)
        and IMAGE_RE.fullmatch(runtime["image_digest"]) is not None,
        "CTA runtime image digest is invalid",
    )
    _require(
        runtime["localhost_marker_verified"] is True,
    ...
    _require(
        control["production_registry_experiments"] == [manifest["experiment_id"]],
        "runtime Production registry is not CTA-only",
    )
    _require(
        control["cta_events_before_start"] == 0, "CTA events exist before activation"
    )
    _require(
        control["aa_production_allocation_percent"] == 0,
        "A/A Production allocation is nonzero",
    )
    _require(
        control["cta_production_allocation_percent"] == 0,
        "CTA Production allocation is nonzero before start",
    )
    ...
    safety = _exact_object(
        root["safety"],
        {
            ...
            "meta_ads_mutated",
            "biznisweb_mutated",
            "price_product_cart_checkout_order_mutated",
        },
        "runtime safety",
    )
    _require(
        not any(safety.values()),
        "CTA runtime observation contains unsafe data or mutation",
    )


def validate_start_observation(
    observation: Mapping[str, Any], manifest: Mapping[str, Any]
) -> None:
    root = _exact_object(
        observation,
    ...
    _require(
        root["evidence_type"] == "vevo_growthbook_cta_activation_readback",
        "CTA start evidence type drift",
    )
    _require(
        root["experiment_id"] == manifest["experiment_id"], "CTA start experiment drift"
    )
    ...
    _require(
        ...
        "GrowthBook build is absent",
    )
    _require(
        growthbook["experiment_id"]
        == manifest["launch_contract"]["growthbook_experiment_id"],
        "GrowthBook CTA ID drift",
    )
    _require(
        growthbook["experiment_status"] == "running", "CTA experiment is not running"
    )
    _require(
        growthbook["environment"] == "production_only",
        "CTA experiment environment drift",
    )
    _require(growthbook["traffic_percent"] == 100, "CTA traffic is not 100 percent")
    _require(
        growthbook["variation_weights"] == {"control": 0.5, "brand_contrast": 0.5},
        "CTA live weights drift",
    )
    _positive_int(growthbook["feature_revision"], "growthbook.feature_revision")
    _require(
        growthbook["feature_revision_status"] == "live",
        "CTA feature revision is not live",
    )
    _require(
        growthbook["active_production_experiments"] == [manifest["experiment_id"]],
        "CTA is not the only active Production experiment",
    )
    _require(
        growthbook["aa_status"] == "stopped_zero_allocation",
        "A/A is not stopped at zero allocation",
    )
    _require(
        growthbook["data_source_id"] == "ds_19g6mmt5stlp6", "CTA data source drift"
    )
    _require(
        growthbook["goal_metrics"] == ["vevo_add_to_cart_24h"], "CTA goal metric drift"
    )
    _require(
        growthbook["secondary_metrics"]
        == [
        ...
        "CTA secondary metrics drift",
    )
    _require(
        growthbook["guardrail_metrics"] == CTA_GUARDRAILS,
        "CTA Pro guardrail metrics drift",
    )
    gtm = _exact_object(
        root["gtm"],
        {"container_id", "container_version_id", "unprocessed_changes"},
        "gtm",
    )
    _require(
        gtm
    ...
    qa = _exact_object(
        root["tag_assistant"],
        {
            ...
            "desktop_verified",
            "mobile_verified",
            "consent_accept_reject_withdrawal_verified",
            "control_observed",
            "brand_contrast_observed",
            "cta_css_matches_design_contract",
            "console_error_count",
        },
        "tag_assistant",
    )
    ...
    collector = _exact_object(
        root["collector"],
        {
            ...
            "repeat_exposed_device_count",
            "sticky_consistent_repeat_device_count",
            "sticky_inconsistent_device_count",
            "observed_variations",
        },
        "collector",
    )
    _positive_int(
        collector["accepted_receipt_count"], "collector.accepted_receipt_count"
    )
    _require(
        _positive_int(
            collector["target_exposure_count"], "collector.target_exposure_count"
        )
        >= 2,
        "CTA collector must observe both variations",
    )
    _require(
        _positive_int(
            collector["repeat_exposed_device_count"],
            "collector.repeat_exposed_device_count",
        )
        >= 1,
        "CTA sticky repeat is absent",
    )
    _require(
        collector["sticky_consistent_repeat_device_count"]
        == collector["repeat_exposed_device_count"],
        "CTA sticky repeat parity drift",
    )
    _require(
        collector["sticky_inconsistent_device_count"] == 0,
        "CTA sticky assignment is inconsistent",
    )
    _require(
        collector["observed_variations"] == ["brand_contrast", "control"],
        "CTA observed variation set drift",
    )
    commerce = _exact_object(
        root["commerce"],
        {
            "cta_text_unchanged",
            "cta_dimensions_layout_placement_unchanged",
            "price_unchanged",
    ...
    _require(
        not any(safety.values()),
        "CTA start observation contains unsafe data or mutation",
    )
...
def _validate_post_aa_sources(
    ...
    _require(
        completion.get("aa_pass", {}).get("verdict") == "PASS",
        "CTA activation requires A/A PASS",
    )
    _require(
        completion.get("stop_readback", {}).get("status") == "verified_zero_allocation",
        "CTA activation requires verified zero-allocation A/A stop",
    )
    _require(
        snapshot.get("snapshot_build_allowed") is True,
        "A/A protected snapshot gate was not opened",
    )
    try:
    ...
    _require(
        ...
        "CTA activation requires a frozen sample",
    )
    _require(
        sample.get("activation_allowed") is False,
        "CTA sample freeze may not activate CTA",
    )
    _require(
        lifecycle.get("status") == "verified_completed_aa_21d_lifecycle_preflight",
        "CTA activation requires verified completed-A/A lifecycle preflight",
    )
    _require(
        lifecycle.get("verified") is True
        and lifecycle.get("activation_allowed") is False,
        "CTA lifecycle gate is not safely verified",
    )
    try:
        validate_lifecycle_manifest(lifecycle, lifecycle_observation)
    except CtaEvaluationError as exc:
        raise CtaActivationRecordingError(
            f"CTA lifecycle evidence is invalid: {exc}"
        ) from exc
    _require(
        workspace.get("state")
        == "production_aa_completed_cta_sample_freeze_pro_quantiles_verified",
        "GrowthBook workspace is not post-A/A with verified Pro quantiles",
    )
    _require(
        workspace.get("workspace", {}).get("production_allocation_percent") == 0,
        "GrowthBook workspace allocation is nonzero",
    )
    _require(
    ...
    _require(
        ...
        is False,
        "GrowthBook Production gate is already open",
    )
    experiments = {
    ...
    _require(
        ...
        "GrowthBook experiment set drift",
    )
    _require(
        experiments["vevo-sk-aa-001"].get("production_allocation_percent") == 0,
        "A/A workspace allocation is nonzero",
    )
    ...
    _require(
        ...
        "CTA allocation is nonzero before start",
    )
    _require(
    ...
    clone = (
        workspace.get("athena", {}).get("production", {}).get("growthbook_clone", {})
    )
    _require(
        clone.get("paid_pro_upgrade_authorized") is True,
        "GrowthBook paid Pro upgrade is not recorded",
    )
    production = registry.get("environments", {}).get("production", {})
    preview = registry.get("environments", {}).get("preview", {})
    _require(
        set(production) == {"vevo-sk-product-cta-color-001"},
        "collector Production registry is not CTA-only",
    )
    _require(
        production["vevo-sk-product-cta-color-001"]
        == preview.get("vevo-sk-product-cta-color-001"),
        "collector CTA registry differs from Preview contract",
    )
    ...
    *,
    ...
    for name in EXPECTED_PATHS:
        ...
        if expected_static is not None:
            _require(
                source_hashes[name] == expected_static,
                f"{name} checked-in SHA-256 drift",
            )
    _require(
    ...
    _require(
        ...
        "CTA runtime observation SHA-256 mismatch",
    )
    ...
    _require(
        ...
        "runtime/checked-in registry SHA-256 drift",
    )
    _require(
    ...
    _require(
        lifecycle["observation_sha256"] is not None,
        "CTA lifecycle observation is not bound",
    )
    _require(
    ...
    _require(
        lifecycle["source_aa_snapshot_sha256"] == source_hashes["aa_snapshot"],
...
def _running_workspace(
    ...
    _require(
        updated.get("state")
        == "production_aa_completed_cta_sample_freeze_pro_quantiles_verified",
        "GrowthBook workspace is not ready for CTA start with verified Pro quantiles",
    )
    experiments = {
        row.get("tracking_key"): row
    ...
    _require(
        isinstance(cta, dict) and cta.get("status") == "draft",
        "GrowthBook CTA workspace draft drift",
    )
    _require(
        cta.get("production_allocation_percent") == 0,
        "GrowthBook CTA workspace allocation is nonzero",
    )
    started = _parse_utc(
        observation["assignment_started_at_utc"], "assignment_started_at_utc"
    )
    ...
    cta.update(
        {
            ...
            "started_date": started.astimezone(ZoneInfo("Europe/Bratislava"))
            .date()
            .isoformat(),
            ...
            "activation_evidence": {
                ...
                "target_total_sample": manifest["launch_contract"][
                    "target_total_sample"
                ],
...
def record_start(
    ...
    *,
    ...
    _require(
        ...
        == observation_sha256,
        "CTA start observation SHA-256 mismatch",
    )
    validate_start_observation(observation, manifest)
    production = registry.get("environments", {}).get("production", {})
    _require(
        set(production) == {manifest["experiment_id"]},
        "CTA start registry is not CTA-only",
    )
...
def validate_running_handoff(
    ...
    _require(
        digest == manifest["start_readback"]["observation_sha256"],
        "CTA start handoff observation SHA-256 drift",
    )
    _require(
        workspace.get("state")
        == "production_cta_running_activation_verified_pro_quantiles_verified",
    ...
    _require(
        workspace.get("workspace", {}).get("production_allocation_percent") == 100,
        "CTA running workspace allocation drift",
    )
    _require(
        workspace.get("decision_gates", {}).get("production_activation_allowed")
        is True,
        "CTA running workspace gate is closed",
    )
    experiments = {
        row.get("tracking_key"): row
        for row in workspace.get("experiments", [])
    ...
    _require(
        cta.get("status") == "running_production_cta_only",
        "CTA workspace experiment is not running",
    )
    _require(
        cta.get("production_allocation_percent") == 100,
        "CTA workspace allocation is not 100 percent",
    )
    _require(
        cta.get("feature_rule_revision")
        == observation["growthbook"]["feature_revision"],
        "CTA workspace feature revision drift",
    )
    ...
    _require(
        set(production) == {manifest["experiment_id"]},
        "CTA running collector registry is not CTA-only",
    )
    _require(
        production[manifest["experiment_id"]] == preview.get(manifest["experiment_id"]),
        "CTA running collector contract differs from Preview",
    )
...
def _parser() -> argparse.ArgumentParser:
    ...
    open_parser.add_argument(
        "--aa-snapshot", type=Path, default=DEFAULT_AA_SNAPSHOT_PATH
    )
    open_parser.add_argument(
        "--pro-upgrade", type=Path, default=DEFAULT_PRO_UPGRADE_PATH
    )
    open_parser.add_argument(
    ...
    start_parser.add_argument(
        "--completion", type=Path, default=DEFAULT_COMPLETION_PATH
    )
    start_parser.add_argument(
        "--aa-snapshot", type=Path, default=DEFAULT_AA_SNAPSHOT_PATH
    )
    ...
    try:
        ...
        if args.command == "open-review":
            ...
            runtime = _load_canonical(
                ...
                args.runtime_observation_sha256,
            ...
            print(
                "VEVO_CTA_START_REVIEW_OPENED:manual_growthbook_start=true:automatic_mutation=false"
            )
            return 0
        ...
        print(
            "VEVO_CTA_START_RECORDED:production_allocation=100:first_n_frozen=true:automatic_mutation=false"
        )
        return 0
    except (
