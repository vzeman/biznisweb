...
if [[ "${PROJECT}" != "vevo" ]]; then
  echo "GrowthBook Preview host gate is VEVO-only, got ${PROJECT}" >&2
  exit 1
fi
if [[ "${ENVIRONMENT}" != "preview" ]]; then
  echo "GrowthBook Preview host gate requires preview, got ${ENVIRONMENT}" >&2
  exit 1
fi
if [[ "${REPO_ROOT}" != "/app" ]]; then
  echo "GrowthBook Preview host gate requires /app, got ${REPO_ROOT}" >&2
  exit 1
fi
...
with TemporaryDirectory(prefix="vevo-growthbook-host-gate-", dir="/tmp") as directory:
    ...
    health = {
        ...
        "project": project,
        "service": "vevo-growthbook-preview-reconciliation",
    }
    marker = {
