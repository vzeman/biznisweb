#!/usr/bin/env python3
"""
Export, generate, apply, and verify short SEO content for noze.sk AF categories.

The noze.sk shop is managed through the ROY BiznisWeb GraphQL endpoint. The AF
language version uses lang_id=48 and public links redirect from roy.sk/af to
noze.sk category URLs.
"""
...
DEFAULT_OUTPUT = ROOT / "projects" / "roy" / "exports" / "noze-af-category-seo-2026-06-11.json"

AF_LANG_ID = "48"
AF_LANG_CODE = "af"
NOZE_ROOT_CATEGORY_ID = "5958"
...
def noze_url(link: str) -> str:
    ...
    if path.startswith("/af/"):
        path = path[3:]
    return f"https://www.noze.sk{path}"
...
class BiznisWebClient:
    def __init__(self, api_url: str, api_token: str) -> None:
        ...
        self.session.headers.update({
            ...
            "User-Agent": "biznisweb-reporting/roy-noze-af-category-seo",
        })
    ...
    def get_category(self, category_id: str) -> Dict[str, Any]:
        data = self.graphql(GET_CATEGORY_QUERY, {"id": str(category_id)})
    ...
    def update_category(self, category_id: str, payload: Dict[str, str]) -> Dict[str, Any]:
        data = self.graphql(
            UPDATE_CATEGORY_MUTATION,
            {"id": str(category_id), "langId": AF_LANG_ID, "data": payload},
        )
...
class BiznisWebAdminClient:
    def __init__(self, base_url: str, username: str, password: str) -> None:
        ...
        self.session = requests.Session()
        self.session.headers.update({
            "User-Agent": "biznisweb-reporting/roy-noze-af-category-seo-admin",
        })
...
def build_seo(node: CategoryNode) -> Dict[str, str]:
    ...
    seo_title = trim_to(f"{public_title} | noze.sk", 60)
    seo_base = f"{public_title} na noze.sk: výber podľa {context['choice']}."
    seo_tail = " Prehľadný sortiment a praktické parametre."
    if len(seo_base + seo_tail) <= 155:
        ...
        seo_description = trim_to(
            f"{public_title} na noze.sk: prehľadný výber podľa použitia, materiálu a parametrov.",
            155,
        )
    return {
...
def traverse_categories(client: BiznisWebClient, root_id: str) -> List[CategoryNode]:
    ...
    while queue:
        ...
        if str(language.get("id")) != AF_LANG_ID or str(language.get("code")).lower() != AF_LANG_CODE:
        ...
        for child in category.get("children_categories") or []:
            ...
            if str(child_lang).lower() == AF_LANG_CODE:
    ...
    for url in list(urls)[:limit]:
        try:
            ...
            results.append({
                ...
                "has_noze_domain": urlparse(response.url).netloc.endswith("noze.sk"),
...
def main() -> int:
    configure_stdout()
    parser = argparse.ArgumentParser(description="Generate/apply noze.sk AF category SEO content.")
    ...
    parser.add_argument("--root-category-id", default=NOZE_ROOT_CATEGORY_ID)
    ...
    payload = {
        ...
        "target": {
            ...
            "language_id": AF_LANG_ID,
            "language_code": AF_LANG_CODE,
            "language_name": "Afghanistan",
            "public_domain": "https://www.noze.sk",
    ...
    print(f"target_language={AF_LANG_CODE.upper()} lang_id={AF_LANG_ID}")
    ...
    if public_checks:
        ok = sum(1 for item in public_checks if item.get("status") == 200 and item.get("has_noze_domain"))
