...
Unchanged order/status IDs with changed labels and matching current API/native catalogues support a shop status rename, not a client-language transport defect. No VEVO role is inferred from a ROY ID. Duplicate labels on 33/34 must not merge their roles. On September 14 the owner explicitly confirmed that ID1 `New order` was `Čaká na vybavenie`; the fresh same-shop catalogue and private incident proof `report-alarm-incident-012629Z.json` bind this additional role. The shared v2 contract now includes it.
...
The branch now consumes the reviewed shared contract from `10d705eeb3d9549dd0383f9460468fa51fe6041d`. Live VEVO acquisition binds its same-shop catalogue before inventory or cached rows are classified. Period child exporters receive the same verified catalogue without extra catalogue reads. Cache acceptance validates every row even when payment metadata is already present; an unreviewed renamed label invalidates the day and the existing bulk reader retrieves current rows. The temporary-cache regression preserves complete included/excluded accounting and keeps fresh label drift strict; the owner-confirmed ID1 rename is accepted, while unknown-role label drift still invalidates stale cache rows. No old VEVO report cache was observed in the inspected audit worktrees, and Docker excludes local data. Raw order/cache/display rows remain unchanged; classification uses canonical copies. Unknown inactive IDs cannot receive a reviewed paid/shipped role by name. The classification normalizer also returns an empty label for an unbound row; raw presentation remains unchanged. Actual generic-filter and failed-payment tests prevent this boundary from being bypassed. ROY keeps its existing project policy.

Realized revenue, failed-payment segmentation and shipped-creditnote decisions share the binding. Historical shipped evidence with a status ID uses the same contract; existing old-name-only evidence remains readable without treating a new unbound English label as proof. Internal creditnote contexts carry the bound client only for local classification; exported audit rows and summaries do not serialize it, verified with nonempty JSON-serializable audit output. Acquisition never invokes invoice, status or email mutations.
