...
Status: A/A is configured and running only in staging; the unpublished Preview runtime and one controlled real Preview partition are verified end to end, while recurring reconciliation, same-population Meta/reporting verification, and three paid-Pro p75 metrics remain pending
...
16. Before A/A Production traffic, clone the verified Preview data-source/fact/metric objects onto the separate Production Athena database and workgroup. Never repoint the Preview connection in place.
...
- The controlled `2026-08-21` Preview partition is reconciled and identity-verified through the exact GrowthBook Athena workgroup. A reviewed recurring reconciliation schedule is not configured yet, so ongoing A/A population agreement is not claimed.
