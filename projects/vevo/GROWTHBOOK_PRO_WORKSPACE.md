...
Reconcile the controlled accepted exposure through raw → curated facts → Athena, then run the GrowthBook A/A query and compare collector, curated, assignment-query, and GrowthBook device counts. Keep the reconciler dry-run by default and add a reviewed recurring schedule before real A/A population collection. Do not click GTM `Submit`, do not start CTA A/B, and keep Production allocation at `0%` until this population gate and the ad-block delivery design are approved.
