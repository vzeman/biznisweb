...
9. **Current step:** merge and run the explicit performance-fact mode of the protected Preview workflow against the existing synthetic A/A device, then create the second fact table. The mode adds one deterministic `lcp_ms=1300` event, reuses the exact existing exposure/device, is idempotent on retry, and repeats the Fargate localhost/marker plus curated-S3/Athena hard gates. Production allocation remains `0%`.
10. **Partially completed:** `VEVO Device Outcomes v1` (`ftb_19g6mmt2dhrdi`) was created from `growthbook_sql/device_outcomes.sql`, identified by `device_id`, and its GrowthBook/Athena query returned the exact one-row anonymous synthetic outcome. `VEVO Performance Vitals v1` was not persisted because its structurally valid query returned no rows; create it from `growthbook_sql/performance_vitals.sql` only after the protected performance fact readback succeeds.
...
- The GrowthBook data source, assignment query, and device-outcome fact table are verified. The performance fact table and all pre-registered metrics remain pending; the UI correctly refused to persist the empty performance definition before a performance fact exists.
