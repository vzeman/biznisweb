...
`.github/workflows/check-vevo-growthbook-production-aa-window.yml` schedules both UTC equivalents of `04:30 Europe/Bratislava` and admits only the correct DST slot after the exact due checkpoint. Before the first due date, after resolution, on the wrong slot, or for an already committed index, it skips before AWS credentials and the population query. The calendar-derived scheduled index preserves the exact frozen daily boundary even if the local PC is off; manual `confirm_checkpoint=true` remains a same-window fallback. Each admitted run selects the exact reconciliation from the bounded success marker plus Scheduler-authenticated CloudTrail `RunTask` event. It prefers retained ECS state and its private IP, but schema `2` explicitly records `cloudtrail_run_task_retention_recovery`, `runtime_state_retained=false`, and `private_ip=null` when the short-lived stopped-task state has expired. That fallback is sufficient only for this read-only checkpoint and never satisfies the live-IP plus localhost-marker hard gate required before an infrastructure mutation. Artifacts are retained for 90 days. Independently read the successful run ID and exact main commit, download its single `vevo-growthbook-aa-window-checkpoint` artifact, verify that the ZIP contains only `vevo-growthbook-aa-window-checkpoint.json`, and record it offline on a new branch. After multi-day PC downtime, record artifacts in checkpoint-index order and stop at the earliest one with at least `1,000`; ignore later captured artifacts once that earlier boundary resolves the window:
