...
`.github/workflows/check-vevo-growthbook-production-aa-window.yml` schedules both UTC equivalents of `04:30 Europe/Bratislava` and admits only the correct DST slot after the exact due checkpoint. Before the first due date, after resolution, on the wrong slot, or for an already committed index, it skips before AWS credentials and the population query. The calendar-derived scheduled index preserves the exact frozen daily boundary even if the local PC is off; manual `confirm_checkpoint=true` remains a same-window fallback. If an exact next checkpoint artifact was never created because its scheduled and same-window runs failed, dispatch the same workflow only after that daily gate has closed with both `confirm_checkpoint=true` and `confirm_historical_backfill=true`. Schema `3` binds the run as `manual_historical_backfill`, reconstructs only the next missing preregistered historical cutoff, and the offline recorder still rejects gaps or later-index selection. Do not use backfill when a successful artifact for that index already exists. Each admitted run selects the exact reconciliation from the bounded success marker plus Scheduler-authenticated CloudTrail `RunTask` event. It prefers retained ECS state and its private IP, while the retained schema `2` compatibility path and schema `3` both record `cloudtrail_run_task_retention_recovery`, `runtime_state_retained=false`, and `private_ip=null` when the short-lived stopped-task state has expired. That fallback is sufficient only for this read-only checkpoint and never satisfies the live-IP plus localhost-marker hard gate required before an infrastructure mutation. Artifacts are retained for 90 days. Independently read the successful run ID and exact main commit, download its single `vevo-growthbook-aa-window-checkpoint` artifact, verify that the ZIP contains only `vevo-growthbook-aa-window-checkpoint.json`, and record it offline on a new branch. After multi-day PC downtime, record artifacts in checkpoint-index order and stop at the earliest one with at least `1,000`; ignore later captured artifacts once that earlier boundary resolves the window:
...
Review and merge that manifest transition through PR. A checkpoint below `1,000` changes only the hash-bound history. The first qualifying checkpoint additionally resolves the exact through-boundary and copies it to both still-disabled component manifests; it does not open a producer or snapshot gate. The later automated component, reviewed manual-QA component, and assembled snapshot/decision/provenance artifacts are each retained for 90 days so extended local-PC downtime cannot break this hash-bound handoff.

## Phase 6 — PASS-bound A/A completion and CTA handoff

Do not stop the Production A/A merely because the minimum date or sample was reached. First assemble the exact protected snapshot and require its independently recomputed decision to be `PASS`. Download the single `vevo-growthbook-aa-snapshot` artifact from its successful `main` workflow run and require exactly `vevo-growthbook-aa-snapshot.json`, `vevo-growthbook-aa-decision.json`, and `vevo-growthbook-aa-provenance.json`. Independently calculate all three SHA-256 values. The canonical provenance must bind the exact repository, workflow, first run attempt, main commit, artifact name, both output hashes, and both source-component run/commit/hash identities. Then run the offline transition on a new branch:

```text
python scripts/record_growthbook_aa_completion.py --output projects/vevo/growthbook_production_aa_completion.json record-pass --snapshot <downloaded-artifact>/vevo-growthbook-aa-snapshot.json --decision <downloaded-artifact>/vevo-growthbook-aa-decision.json --provenance <downloaded-artifact>/vevo-growthbook-aa-provenance.json --snapshot-sha256 <snapshot-sha256> --decision-sha256 <decision-sha256> --provenance-sha256 <provenance-sha256> --workflow-run-id <snapshot-run-id> --main-commit <snapshot-head-sha>
