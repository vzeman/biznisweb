...
GrowthBook 5.0.1 cannot create or update a fact-table SQL definition when the test returns zero rows, because its UI then reports that no columns were returned. The Production-only `device_outcomes_production.sql` and `performance_vitals_production.sql` therefore append exactly one metadata-only schema probe guarded by `WHERE '{{ experimentId }}' = '%'`. The frozen Preview SQL files remain unchanged. GrowthBook substitutes `%` only for its generic schema test; a named A/A or A/B analysis substitutes the exact experiment ID, making the probe branch false. The probe uses an impossible all-zero UUID-shaped identifier, the reserved experiment ID `__growthbook_schema_only__`, no Meta dimensions, and `NULL` metric values. It is generated by Athena from a literal `VALUES` row, is never written to S3 or reporting, and cannot join a named experiment assignment. Before Production traffic, GrowthBook's fact-table UI test must return exactly one probe row while the underlying curated branch remains exactly zero rows.
...
Non-realized pending/cancelled/refunded rows currently carry zero order value while preserving their explicit lifecycle counters. The A/A pipeline can validate joins and lifecycle changes with that conservative rule, but a final A/B CM1 decision remains a `NO-GO` until the complete source window has passed both the 7-day order-attribution window and the 14-day per-order credit-note/refund-cost checkpoint. This is now machine-enforced by `growthbook_cta_lifecycle_reconciliation.json`, the source-explicit protected `collect-vevo-growthbook-cta-lifecycle-preflight.yml`, `scripts/record_growthbook_cta_lifecycle_reconciliation.py`, and `scripts/evaluate_growthbook_cta.py`: the preflight uses only the completed/stopped Production A/A cohort, compares canonical direct-curated facts with one aggregate Athena read after 21 days, and forbids CTA outcomes. The recorder requires canonical identity-free evidence, an independent SHA-256, exact workflow run/main commit, cent-level CM1 parity, mature lifecycle cases, and a hash-bound curated reporting-quality object; the evaluator re-opens that exact observation and verifies its canonical SHA-256 and manifest parity. The experiment snapshot cannot self-assert this gate.
