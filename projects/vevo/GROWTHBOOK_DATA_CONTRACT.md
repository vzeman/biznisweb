...
Non-realized pending/cancelled/refunded rows currently carry zero order value while preserving their explicit lifecycle counters. The A/A pipeline can validate joins and lifecycle changes with that conservative rule, but a final A/B CM1 decision remains a `NO-GO` until the 14-day credit-note/refund-cost reconciliation is verified against the existing reporting audit. This is now machine-enforced by `growthbook_cta_lifecycle_reconciliation.json`, `scripts/record_growthbook_cta_lifecycle_reconciliation.py`, and `scripts/evaluate_growthbook_cta.py`: the recorder requires canonical identity-free evidence, an independent SHA-256, exact cent-level CM1 parity, mature lifecycle cases, and a hash-bound curated reporting-quality object; the evaluator re-opens that exact observation and verifies its canonical SHA-256 and manifest parity. The experiment snapshot cannot self-assert this gate.
