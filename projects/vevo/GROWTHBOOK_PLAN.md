...
- The safety-only checkpoint boundary is now frozen separately in `growthbook_cta_safety_monitoring.json`. `scripts/evaluate_growthbook_cta_safety.py` accepts only two aggregate variation-health rows, explicit unchanged-commerce readbacks, exact data-quality flags, and no identities, primary/business outcomes, Meta dimensions, or winner fields. It enforces the pre-registered 200 measured page loads per arm, LCP/INP/CLS, and client-error thresholds; a reproducible cart/checkout runtime error or price/cart/order mutation can require an immediate manual stop before performance maturity. Its only decisions are `CONTINUE`, `CONTINUE_NOT_MATURE`, and `STOP_REQUIRED`; it has no external client and always leaves every automatic mutation and winner boundary false. The checked-in state remains `waiting_for_verified_cta_start`, with collection, recording, and manual-stop integration still closed until a later reviewed implementation binds the verified CTA start. Therefore this contract alone does not claim operational early-safety monitoring.
