...
`growthbook_cta_lifecycle_reconciliation.json` is a separate reviewed prelaunch evidence gate for the known refund/credit-note value boundary. It is currently pending, false, identity-free, and activation-disabled. Its protected producer is the main-only, daily PC-independent `.github/workflows/collect-vevo-growthbook-cta-lifecycle-preflight.yml`, and its only source cohort is the completed/stopped Production A/A `vevo-sk-aa-001`; CTA outcomes cannot exist yet and are explicitly forbidden. Scheduled runs skip successfully before AWS until A/A PASS/stop/window and 21 days after the exact A/A window end (7-day attribution plus 14-day per-order maturity) are recorded; an exactly confirmed manual run is the fallback. The workflow compares canonical curated facts reduced directly with one aggregate Athena read, binds the retained reporting-quality object to the exact direct cohort generation rather than a newer unrelated report, requires one facts generation, zero immature orders, at least one mature cancellation/refund/credit-note case, matching lifecycle counts, and cent-exact CM1 parity, then shreds all temporary identity-bearing files. `scripts/record_growthbook_cta_lifecycle_reconciliation.py` accepts only the single canonical identity-free artifact plus independently supplied SHA-256, workflow run ID, exact main commit, and verification time. It writes the versioned observation and may change only the allowlisted manifest fields; a verified manifest is rejected unless the observation file exists and its canonical SHA-256/provenance/source hashes match. A final CTA decision cannot use CM1, cancellation, or refund outcomes until this gate passes, and a boolean copied into an experiment snapshot cannot bypass it.
