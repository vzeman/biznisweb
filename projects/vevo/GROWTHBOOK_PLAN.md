...
- The final look is separately versioned by `growthbook_cta_final_snapshot.json`, `growthbook_sql/cta_final_snapshot_production.sql`, and the main-only `build-vevo-growthbook-production-cta-final-snapshot.yml`. It remains closed until the verified stop recorder binds all source hashes and the full 14-day follow-up is due. The workflow refuses any second outcome-query attempt, including after a failed or cancelled run whose query step had started; it first verifies the exact account/stack/schedule/task definition/image/service `/app` contract, post-due scheduled reconciliation, marker parity, alarms, DLQ, one diagnostic Fargate task/private IP, and direct localhost health/runtime markers, then runs exactly one Athena query. Raw device IDs are used only inside Athena for deterministic first-`N` selection; the only result is two aggregate arm rows and the only artifact is the canonical identity-free snapshot plus decision. `record_growthbook_cta_final_snapshot.py` independently re-evaluates the snapshot byte-for-byte, records only `WIN`, `LOSE`, or `INCONCLUSIVE`, closes all result-read gates, and leaves every recommendation unapplied pending a separate manual review.
