...
REPORT_FILE = Path(
    "content/VEVO_CONTENT/exports/batch-39-test-gsm-2026-07-14-publication-verify.json"
)
USER_AGENT = "Codex VEVO batch 39 test public verification"
...
def visible_text(markup):
    return re.sub(
        r"\s+", " ", html.unescape(TAG_RE.sub(" ", markup))
    ).strip()
...
def article_segment(page_source):
    ...
    end = min(ends) if ends else min(len(page_source), start + 160_000)
...
def main():
    ...
    for article in articles:
        ...
        internal_article_links = expected_links(article, "/n/")
        ...
        checks = {
            ...
            "product_links_present": all(
                href in segment for href in product_links
            ),
            "category_links_present": all(
                href in segment for href in category_links
            ),
            "internal_article_links_present": all(
                href in segment for href in internal_article_links
            ),
            "external_sources_present": all(
                href in segment for href in external_links
            ),
            "bad_escaped_html": bool(
                re.search(
                    r"&lt;/?(?:p|h2|h3|div|table|a)\b", segment, re.IGNORECASE
                )
            ),
            ...
            "internal_article_links": internal_article_links,
        ...
        checks["ok"] = all(
            [
                ...
                checks["visible_word_count"] >= 1800,
                checks["table_count"] >= 2,
                checks["styled_block_count"] >= 7,
                checks["product_links_present"],
                checks["category_links_present"],
                checks["internal_article_links_present"],
    ...
    if not output["all_ok"]:
        raise SystemExit("Batch 39 public verification failed")
