#!/usr/bin/env python3
"""Independently verify VEVO batch 44 on the public website."""
...
ARTICLES_JSON = Path("content/VEVO_CONTENT/imports/batch-44-2026-08-20-articles.json")
OUT_JSON = Path(
    "content/VEVO_CONTENT/exports/batch-44-2026-08-20-publication-verify.json"
)
USER_AGENT = "Codex VEVO batch 44 independent public verification"
...
def article_segment(page_source: str) -> str:
    ...
    end = min(end_candidates) if end_candidates else min(len(page_source), start + 360_000)
...
def text_variant_present(value: str, page: str) -> bool:
    ...
    return value in decoded or mojibake_variant(value) in decoded
...
def verify_article(article: dict[str, object]) -> dict[str, object]:
    ...
    checks: dict[str, object] = {
        ...
        "clean_url_ok": urlparse(response.url).path.rstrip("/") == f"/n/{slug}",
...
def main() -> None:
    ...
    if not output["all_ok"]:
        ...
        raise SystemExit("Batch 44 public verification failed")
