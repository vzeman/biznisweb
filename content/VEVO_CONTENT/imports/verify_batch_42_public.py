...
ARTICLES_JSON = Path("content/VEVO_CONTENT/imports/batch-42-2026-07-22-articles.json")
OUT_JSON = Path("content/VEVO_CONTENT/exports/batch-42-2026-07-22-publication-verify.json")
USER_AGENT = "Codex VEVO batch 42 independent public verification"
...
def article_segment(page_source):
    ...
    end = min(end_candidates) if end_candidates else min(len(page_source), start + 240_000)
...
def main():
    ...
    if not output["all_ok"]:
        ...
        raise SystemExit("Batch 42 public verification failed")
