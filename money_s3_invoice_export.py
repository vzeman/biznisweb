...
def fetch_money_s3_invoice_count(project: str, date_from: date, date_to: date) -> int:
...
def download_money_s3_invoice_export(project: str, date_from: date, date_to: date) -> Tuple[bytes, str, int]:
    base_url, session, arf = _login_admin(project)
    filters = money_s3_invoice_filters(date_from, date_to)
    mass_filter = biznisweb_php_serialize(filters)
