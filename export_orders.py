...
class BizniWebExporter:
    ...
    def _build_product_expense_coverage_qa(
        ...
        recent_30d_fallback_profit: Optional[float] = None
        recent_30d_revenue_share_pct: Optional[float] = None
        recent_30d_date_from: Optional[str] = None
        recent_30d_date_to: Optional[str] = None
        recent_30d_available = False
        ...
        if "purchase_date" in item_df.columns:
            ...
            configured_report_start = pd.to_datetime(report_date_from, errors="coerce")
            configured_recent_end = pd.to_datetime(report_date_to, errors="coerce")
            if pd.notna(configured_recent_end):
                recent_end = pd.Timestamp(configured_recent_end)
                if recent_end.tzinfo is not None:
                    recent_end = recent_end.tz_localize(None)
                recent_end = recent_end.normalize()
            elif not valid_dates.empty:
                recent_end = valid_dates.max()
            else:
                recent_end = None
            if pd.notna(configured_report_start):
                report_start = pd.Timestamp(configured_report_start)
                if report_start.tzinfo is not None:
                    report_start = report_start.tz_localize(None)
                report_start = report_start.normalize()
            elif not valid_dates.empty:
                report_start = valid_dates.min()
            else:
                report_start = None
            if recent_end is not None and report_start is not None:
                recent_30d_available_days = max((recent_end - report_start).days + 1, 0)
                recent_30d_available = recent_30d_available_days >= 30
            if recent_end is not None and recent_30d_available:
        ...
        zero_revenue_gift_missing_items: List[Dict[str, Any]] = []
        zero_revenue_gift_missing_df = item_df.loc[zero_revenue_gift_missing_mask].copy()
        if not zero_revenue_gift_missing_df.empty:
            grouped_gifts = (
                zero_revenue_gift_missing_df.groupby(["product_sku", "item_label"], dropna=False)
                .agg(
                    rows=("order_num", "size"),
                    units=("item_quantity", "sum"),
                    revenue=("item_total_without_tax", "sum"),
                ...
                .reset_index()
                .sort_values(["rows", "units"], ascending=[False, False])
                .reset_index(drop=True)
            )
            for row in grouped_gifts.to_dict("records"):
                zero_revenue_gift_missing_items.append(
                    {
                        "category": "zero_revenue_gift",
                        "product_sku": row.get("product_sku"),
                        "item_label": row.get("item_label"),
                        "rows": int(row.get("rows") or 0),
                        "units": round(float(row.get("units") or 0.0), 2),
                        ...
                        "total_revenue_share_pct": 0.0,
                    }
                )

        missing_cost_items = [
            {"category": "fallback_estimate", **row}
        ...
        if fallback_rows > 0:
            ...
            if recent_30d_revenue_share_pct is not None and recent_30d_revenue_share_pct >= 3:
                warnings.append(
        ...
        zero_revenue_gift_missing_rows = int(zero_revenue_gift_missing_mask.sum())
        zero_revenue_gift_missing_units = float(
            item_df.loc[zero_revenue_gift_missing_mask, "item_quantity"].sum()
        )
        if zero_revenue_gift_missing_rows > 0:
            warnings.append(
                f"{zero_revenue_gift_missing_rows} zero-revenue gift row(s) across "
                f"{len(zero_revenue_gift_missing_items)} product(s) have no mapped purchase cost; "
                "they contribute EUR 0 revenue and EUR 0 fallback profit."
            )
        ...
        return {
            ...
            "fallback_recent_30d_total_revenue": (
                round(recent_30d_total_revenue, 2) if recent_30d_total_revenue is not None else None
            ),
            "fallback_recent_30d_revenue": (
                round(recent_30d_fallback_revenue, 2) if recent_30d_fallback_revenue is not None else None
            ),
            "fallback_recent_30d_profit_before_ads": (
                round(recent_30d_fallback_profit, 2) if recent_30d_fallback_profit is not None else None
            ),
            "fallback_recent_30d_revenue_share_pct": recent_30d_revenue_share_pct,
            "fallback_product_count": len(fallback_items),
            "missing_cost_product_entry_count": len(missing_cost_items),
            "unknown_source_rows": unknown_source_rows,
            "zero_revenue_gift_missing_cost_rows": zero_revenue_gift_missing_rows,
            "zero_revenue_gift_missing_cost_units": round(zero_revenue_gift_missing_units, 2),
            "zero_revenue_gift_missing_cost_product_count": len(zero_revenue_gift_missing_items),
            "source_mix": source_mix_rows,
            "fallback_items": fallback_items,
            "zero_revenue_gift_missing_cost_items": zero_revenue_gift_missing_items,
    ...
    def flatten_order(self, order: Dict[str, Any]) -> List[Dict[str, Any]]:
        ...
        if items:
            ...
            for item in items:
                ...
                resolved_expense_per_item, resolved_expense_source = self._resolve_product_expense(
                    product_sku,
                    item_label,
                    import_code=item_import_code,
                ...
                if is_zero_revenue_gift_exception:
                    ...
                    if expense_per_item is None:
                        if is_zero_revenue_line:
                            expense_per_item, expense_source = self._missing_cost_expense(
                                item_total_without_tax,
                                item_quantity,
                            )
                        elif force_zero_cost:
                            expense_per_item = 0.0
    ...
    def calculate_clv_and_return_time(
        ...
        for week in analysis_weeks:
            ...
            fb_cac = round(week_fb_spend / new_customers, 2) if new_customers > 0 else None
            cac = round(week_paid_spend / new_customers, 2) if new_customers > 0 else None
            
            # Calculate LTV/CAC ratio
            ltv_cac_ratio = round(avg_clv / cac, 2) if cac is not None and cac > 0 else None
        ...
        for idx, row in weekly_clv_df.iterrows():
            ...
            avg_cac = cumulative_paid_spend / cumulative_new_customers if cumulative_new_customers > 0 else None
            cumulative_cac.append(round(avg_cac, 2) if avg_cac is not None else None)
    ...
    def analyze_advanced_dtc_metrics(
        ...
        paid_cac_fb = (paid_spend_fb / new_customers) if new_customers > 0 else None
        paid_cac_blended = (paid_spend_blended / new_customers) if new_customers > 0 else None
        contribution_ltv_cac = (
            contribution_ltv / paid_cac_blended
            if paid_cac_blended is not None and paid_cac_blended > 0
            else None
        )
        ...
        result = {
            'summary': {
                ...
                'paid_cac_fb': round(paid_cac_fb, 2) if paid_cac_fb is not None else None,
                'paid_cac': round(paid_cac_blended, 2) if paid_cac_blended is not None else None,
                'paid_cac_scope': 'tracked_ads_fb_google',
                'contribution_ltv_cac': (
                    round(contribution_ltv_cac, 2) if contribution_ltv_cac is not None else None
                ),
        ...
        contribution_ltv_cac_value = result['summary']['contribution_ltv_cac']
        contribution_ltv_cac_display = (
            f"{contribution_ltv_cac_value:.2f}x"
            if contribution_ltv_cac_value is not None
            else "N/A"
        )
        print(
            f"Advanced DTC metrics complete: first-order margin={result['summary']['first_order_contribution_margin_pct']:.2f}%, "
            f"contribution LTV/CAC={contribution_ltv_cac_display}"
        )
        return result
    ...
    def analyze_roy_product_demand_analytics(
        ...
        if inventory_enabled and inventory_status != "error":
            ...
            inventory_products_df["bundle_component_confirmed_trend_flag"] = False
            inventory_products_df["bundle_component_economic_demand_units_30d"] = 0.0
            inventory_products_df["bundle_component_expected_order_rate_per_day"] = 0.0
            ...
            inventory_products_df["bundle_component_source_contribution"] = 0.0
            ...
            if bundle_component_rules:
                for rule in bundle_component_rules:
                    ...
                    bundle_contribution = float(
                        inventory_products_df.loc[bundle_mask, "profit_without_fixed"].sum()
                    )
                    ...
                    bundle_confirmed_trend = bool(
                        inventory_products_df.loc[
                            bundle_mask,
                            ["trend_confirmed_flag", "confirmed_repeated_bulk_flag"],
                        ]
                        .fillna(False)
                        .astype(bool)
                        .any(axis=None)
                    )
                    bundle_economic_demand_units_30d = 0.0
                    bundle_economic_order_rate_per_day = 0.0
                    for _, bundle_row in inventory_products_df.loc[bundle_mask].iterrows():
                        recent_units = max(float(bundle_row.get("recent_30d_units") or 0.0), 0.0)
                        recent_90d_units = max(float(bundle_row.get("recent_90d_units") or 0.0), 0.0)
                        forecast_units = max(float(bundle_row.get("forecast_30d_units") or 0.0), 0.0)
                        robust_baseline_units = max(
                            float(bundle_row.get("robust_baseline_30d_units") or 0.0),
                            0.0,
                        )
                        confidence = str(bundle_row.get("forecast_confidence") or "low").strip().lower()
                        uplift_weight = forecast_uplift_weights.get(
                            confidence,
                            forecast_uplift_weights["low"],
                        )
                        confirmed_source_trend = bool(
                            bundle_row.get("trend_confirmed_flag")
                            or bundle_row.get("confirmed_repeated_bulk_flag")
                        )
                        if bool(bundle_row.get("robust_model_applied_flag")):
                            source_economic_units = robust_baseline_units + (
                                max(forecast_units - robust_baseline_units, 0.0)
                                * uplift_weight
                                * float(confirmed_source_trend)
                            )
                        elif bool(bundle_row.get("unusual_large_order_flag")):
                            # Even before the minimum-history gate, use the
                            # robust candidate rather than a raw one-off spike.
                            source_economic_units = robust_baseline_units
                        elif float(bundle_row.get("orders") or 0.0) >= historical_restock_min_orders:
                            source_economic_units = (
                                recent_units
                                + max(forecast_units - recent_units, 0.0) * uplift_weight
                                if recent_units > 0
                                else forecast_units * uplift_weight
                            )
                            if source_economic_units <= 0 and recent_90d_units > 0:
                                source_economic_units = recent_90d_units * (30.0 / 90.0)
                        else:
                            source_economic_units = 0.0
                        bundle_economic_demand_units_30d += max(source_economic_units, 0.0)
                        if source_economic_units > 0:
                            source_typical_order_units = resolve_typical_order_units(
                                bundle_row.get("typical_order_units"),
                                source_economic_units,
                            )
                            bundle_economic_order_rate_per_day += (
                                source_economic_units / 30.0 / source_typical_order_units
                            )
                    ...
                    for component_rule in component_rules:
                        ...
                        inventory_products_df.loc[
                            component_mask,
                            "bundle_component_source_contribution",
                        ] += bundle_contribution
                        inventory_products_df.loc[
                            component_mask,
                        ...
                        inventory_products_df.loc[
                            component_mask,
                            "bundle_component_economic_demand_units_30d",
                        ] += bundle_economic_demand_units_30d * component_qty
                        inventory_products_df.loc[
                            component_mask,
                            "bundle_component_expected_order_rate_per_day",
                        ] += bundle_economic_order_rate_per_day
                        if bundle_confirmed_trend:
                            inventory_products_df.loc[
                                component_mask,
                                "bundle_component_confirmed_trend_flag",
                            ] = True
            ...
            direct_positive_unit_uplift = np.maximum(
                inventory_products_df["forecast_30d_units"]
                - inventory_products_df["recent_30d_units"],
                0.0,
            )
            inventory_products_df["direct_raw_alert_30d_units"] = (
                inventory_products_df["recent_30d_units"]
                + direct_positive_unit_uplift * inventory_products_df["forecast_uplift_weight"]
            )
            no_recent_direct_units_mask = inventory_products_df["recent_30d_units"] <= 0
            inventory_products_df.loc[
                no_recent_direct_units_mask,
                "direct_raw_alert_30d_units",
            ] = (
                inventory_products_df.loc[no_recent_direct_units_mask, "forecast_30d_units"]
                * inventory_products_df.loc[no_recent_direct_units_mask, "forecast_uplift_weight"]
            )
            direct_historical_unit_fallback_mask = (
                inventory_products_df["historical_restock_relevant_flag"]
                & (inventory_products_df["direct_raw_alert_30d_units"] <= 0)
            ...
            inventory_products_df.loc[
                ...
                inventory_products_df.loc[direct_historical_unit_fallback_mask, "recent_90d_units"]
                * (30.0 / 90.0)
            )
            direct_confirmed_forecast_uplift = np.maximum(
                inventory_products_df["forecast_30d_units"]
                - inventory_products_df["robust_baseline_30d_units"],
                0.0,
            )
            inventory_products_df["direct_alert_30d_units"] = inventory_products_df[
                "direct_raw_alert_30d_units"
            ]
            direct_smart_model_mask = inventory_products_df["robust_model_applied_flag"]
            direct_confirmed_trend_mask = (
                inventory_products_df["trend_confirmed_flag"]
                | inventory_products_df["confirmed_repeated_bulk_flag"]
            )
            inventory_products_df.loc[
                direct_smart_model_mask,
                "direct_alert_30d_units",
            ] = (
                inventory_products_df.loc[
                    direct_smart_model_mask,
                    "robust_baseline_30d_units",
                ...
                + (
                    direct_confirmed_forecast_uplift.loc[direct_smart_model_mask]
                    * inventory_products_df.loc[direct_smart_model_mask, "forecast_uplift_weight"]
                    * direct_confirmed_trend_mask.loc[direct_smart_model_mask].astype(float)
                )
            )
            ...
            # Compose the recurring direct and bundle signals independently.
            # This preserves confirmed bundle uplift without allowing a bundle
            # anomaly to scale either the direct demand or its economics.
            inventory_products_df["alert_30d_units"] = (
                inventory_products_df["direct_alert_30d_units"]
                + inventory_products_df["bundle_component_economic_demand_units_30d"]
            )
            direct_typical_order_units = pd.Series(
                [
                    resolve_typical_order_units(
                        row.get("typical_order_units"),
                        row.get("direct_alert_30d_units"),
                    )
            ...
            direct_implied_order_rate = np.where(
                inventory_products_df["direct_alert_30d_units"] > 0,
                inventory_products_df["direct_alert_30d_units"]
                / 30.0
                / direct_typical_order_units,
                0.0,
            )
            direct_expected_order_rate = np.where(
                inventory_products_df["direct_alert_30d_units"] > 0,
                np.maximum(
                    inventory_products_df["order_rate_per_day"].clip(lower=0.0),
                    direct_implied_order_rate,
                ),
                0.0,
            )
            inventory_products_df["combined_order_rate_per_day"] = (
                direct_expected_order_rate
                + inventory_products_df["bundle_component_expected_order_rate_per_day"]
            )
            inventory_products_df["combined_typical_order_units"] = np.where(
                inventory_products_df["combined_order_rate_per_day"] > 0,
                (inventory_products_df["alert_30d_units"] / 30.0)
                / inventory_products_df["combined_order_rate_per_day"],
                0.0,
            )
            ...
            direct_smart_demand_ratio = np.where(
                inventory_products_df["direct_raw_alert_30d_units"] > 0,
                inventory_products_df["direct_alert_30d_units"]
                / inventory_products_df["direct_raw_alert_30d_units"],
                1.0,
            )
            inventory_products_df.loc[direct_smart_model_mask, "alert_30d_revenue"] = (
                inventory_products_df.loc[direct_smart_model_mask, "raw_alert_30d_revenue"]
                * direct_smart_demand_ratio[direct_smart_model_mask]
            )
            # A component sold only inside a bundle has no direct SKU revenue,
            # but its absence still blocks the bundle sale. Carry the bundle's
            # unit economics onto the component demand horizon so those alerts
            # do not incorrectly rank as zero business impact.
            bundle_component_economic_units = np.minimum(
                inventory_products_df["alert_30d_units"],
                inventory_products_df["bundle_component_economic_demand_units_30d"],
            ).clip(lower=0.0)
            bundle_component_unit_revenue = np.where(
                inventory_products_df["bundle_component_total_units"] > 0,
                inventory_products_df["bundle_component_source_revenue"]
                / inventory_products_df["bundle_component_total_units"],
                0.0,
            )
            bundle_component_unit_contribution = np.where(
                inventory_products_df["bundle_component_total_units"] > 0,
                inventory_products_df["bundle_component_source_contribution"]
                / inventory_products_df["bundle_component_total_units"],
                0.0,
            )
            inventory_products_df["bundle_component_alert_30d_revenue_estimate"] = (
                bundle_component_economic_units * bundle_component_unit_revenue
            )
            inventory_products_df["bundle_component_alert_30d_contribution_estimate"] = (
                bundle_component_economic_units * bundle_component_unit_contribution
            )
            ...
            inventory_products_df["alert_30d_contribution_estimate"] = (
                ...
                * inventory_products_df["recent_margin_without_fixed_pct"]
                / 100.0
            )
            # Blocking impact is deliberately separate from additive revenue
            # KPIs: any one missing component can block the full bundle, while
            # summing that full value across every component would multiply the
            # same commercial opportunity in dashboard totals.
            inventory_products_df["stockout_revenue_basis_30d"] = (
                inventory_products_df["alert_30d_revenue"]
                + inventory_products_df["bundle_component_alert_30d_revenue_estimate"]
            )
            inventory_products_df["stockout_contribution_basis_30d"] = (
                inventory_products_df["alert_30d_contribution_estimate"]
                + inventory_products_df["bundle_component_alert_30d_contribution_estimate"]
            )
            ...
            typical_units_for_risk = pd.Series(
                [
                    resolve_typical_order_units(
                        row.get("combined_typical_order_units"),
                        row.get("alert_30d_units"),
                    )
                    for _, row in inventory_products_df.iterrows()
                ],
                index=inventory_products_df.index,
            ...
            implied_order_rate_for_risk = np.where(
                typical_units_for_risk > 0,
                inventory_products_df["daily_units_for_alert"] / typical_units_for_risk,
                0.0,
            )
            order_rate_for_risk = np.maximum(
                inventory_products_df["combined_order_rate_per_day"].clip(lower=0.0),
                implied_order_rate_for_risk,
            )
            ...
            stockout_impact_rows = [
                stockout_business_impact(
                    expected_orders=row.get("lead_time_expected_orders"),
                    available_units=row.get("net_available_quantity"),
                    typical_order_units=row.get("combined_typical_order_units"),
                    alert_units=row.get("alert_30d_units"),
                    alert_revenue=row.get("stockout_revenue_basis_30d"),
                    alert_contribution=row.get("stockout_contribution_basis_30d"),
                )
                for _, row in inventory_products_df.iterrows()
            ]
            stockout_impact_df = pd.DataFrame(
                stockout_impact_rows,
                index=inventory_products_df.index,
                columns=[
            ...
            for column, decimals in (
                ("lead_time_expected_shortage_units", 3),
                ("stockout_contribution_at_risk", 2),
                ("stockout_revenue_at_risk", 2),
            ):
                inventory_products_df[column] = stockout_impact_df[column].round(decimals)

            positive_contribution_impact = inventory_products_df.loc[
                inventory_products_df["stockout_contribution_at_risk"] > 0,
                "stockout_contribution_at_risk",
            ]
            positive_revenue_impact = inventory_products_df.loc[
                inventory_products_df["stockout_revenue_at_risk"] > 0,
                "stockout_revenue_at_risk",
            ]
            contribution_reference = max(
                float(positive_contribution_impact.quantile(0.9))
                if not positive_contribution_impact.empty
                else 0.0,
                1.0,
            )
            revenue_reference = max(
                float(positive_revenue_impact.quantile(0.9))
                if not positive_revenue_impact.empty
                else 0.0,
                1.0,
            )
            inventory_products_df["stockout_pain_score"] = np.clip(
                (
                    np.clip(
                        inventory_products_df["stockout_contribution_at_risk"] / contribution_reference,
                        0.0,
                        1.0,
                    ) * 0.75
                    + np.clip(
                        inventory_products_df["stockout_revenue_at_risk"] / revenue_reference,
                        0.0,
                        1.0,
                    ) * 0.20
                    + inventory_products_df["strategic_stock_flag"].astype(float) * 0.05
                ) * 100.0,
                0.0,
                100.0,
            ).round(1)
            # Keep the established field for downstream compatibility, but its
            # meaning is now the expected CM2/revenue pain of a lead-time
            # stockout instead of scarcity + unit velocity + margin percent.
            inventory_products_df["restock_priority_score"] = inventory_products_df["stockout_pain_score"]
            ...
            revenue_at_risk_rows_df = (
                ...
                .sort_values(
                    ["stockout_revenue_at_risk", "stockout_contribution_at_risk", "stock_risk_rank", "sku"],
    ...
    def calculate_financial_metrics(self, df: pd.DataFrame, date_agg: pd.DataFrame, clv_return_time_analysis: pd.DataFrame = None) -> dict:
        ...
        current_fb_cac = (total_fb_spend / total_new_customers) if total_new_customers > 0 else None
        blended_cac = (total_ad_spend / total_new_customers) if total_new_customers > 0 else None
        ...
        cac_headroom = break_even_cac - blended_cac if blended_cac is not None else None
        cac_headroom_pct = (
            cac_headroom / break_even_cac * 100
            if cac_headroom is not None and break_even_cac != 0
            else None
        )
        contribution_ltv_cac = (
            pre_ad_contribution_per_customer / blended_cac
            if blended_cac is not None and blended_cac > 0
            else None
        )
        ...
        if pre_ad_contribution_per_order > 0 and blended_cac is not None:
            payback_orders = blended_cac / pre_ad_contribution_per_order
        else:
            payback_orders = None
        ...
        if post_ad_contribution_per_order > 0 and blended_cac is not None:
            post_ad_payback_orders = blended_cac / post_ad_contribution_per_order
        else:
        ...
        if clv_return_time_analysis is not None and not clv_return_time_analysis.empty and 'cac' in clv_return_time_analysis.columns:
            for _, row in clv_return_time_analysis.iterrows():
                ...
                cac_week = row.get('cac')
                weekly_payback = (
                    float(cac_week) / pre_ad_contribution_per_order
                    if pd.notna(cac_week) and pre_ad_contribution_per_order > 0
                    else None
                )
                payback_weekly_labels.append(label)
                payback_weekly_orders.append(
                    round(float(weekly_payback), 2) if weekly_payback is not None else None
                )
        ...
        metrics = {
            ...
            'current_fb_cac': round(current_fb_cac, 2) if current_fb_cac is not None else None,
            'paid_cac': round(blended_cac, 2) if blended_cac is not None else None,
            'blended_cac': round(blended_cac, 2) if blended_cac is not None else None,
            'blended_cac_scope': 'tracked_ads_fb_google',
            'cac_headroom': round(cac_headroom, 2) if cac_headroom is not None else None,
            'cac_headroom_pct': round(cac_headroom_pct, 1) if cac_headroom_pct is not None else None,
            'contribution_ltv_cac': (
                round(contribution_ltv_cac, 2) if contribution_ltv_cac is not None else None
            ),
    ...
    def validate_metric_consistency(self, date_agg: pd.DataFrame, financial_metrics: dict, clv_return_time_analysis: pd.DataFrame = None) -> dict:
        ...
        if clv_return_time_analysis is not None and not clv_return_time_analysis.empty:
            ...
            checks['cac_denominator_available'] = bool(total_new_customers > 0)
            if total_new_customers > 0:
                cac_expected = total_cac_spend / total_new_customers
                ...
                if cac_reported is None:
                    cac_delta = None
                    checks['cac_ok'] = False
                else:
                    cac_delta = float(cac_reported) - cac_expected
                    checks['cac_ok'] = abs(cac_delta) <= 0.01
                checks['cac_expected'] = round(cac_expected, 4)
                checks['cac_reported'] = round(float(cac_reported), 4) if cac_reported is not None else None
                checks['cac_delta'] = round(cac_delta, 4) if cac_delta is not None else None
                checks['cac_reason'] = 'reconciled' if checks['cac_ok'] else 'reported_cac_missing_or_mismatched'
            else:
                checks['cac_expected'] = None
                checks['cac_reported'] = None
                checks['cac_delta'] = None
                checks['cac_ok'] = False
                checks['cac_reason'] = 'no_new_customer_denominator'
        else:
            checks['cac_expected'] = None
            checks['cac_reported'] = None
            checks['cac_delta'] = None
            ...
            checks['cac_denominator_available'] = False
            checks['cac_reason'] = 'weekly_acquisition_analysis_unavailable'
            checks['cac_ok'] = False
    ...
    def display_clv_return_time_analysis(self, analysis: pd.DataFrame):
        ...
        for _, row in analysis.iterrows():
            ...
            cac = row.get('cac')
            cac_display = f"{cac:.2f}" if pd.notna(cac) else "N/A"
        ...
        cac_spend_column = 'paid_ads_spend' if 'paid_ads_spend' in analysis.columns else 'fb_ads_spend'
        total_paid_spend = analysis[cac_spend_column].sum() if cac_spend_column in analysis.columns else 0
        overall_cac = total_paid_spend / total_new if total_new > 0 else None
